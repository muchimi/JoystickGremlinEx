...
| To accept inputs from an Elgato Streamdeck | Prefer the built-in [Stream Deck plugin bridge](streamdeck.md) (JG Ex Button in Stream Deck software → GEX). Alternatively use Bitfocus Companion + OSC for the same hardware. |
| To accept inputs from general AV hardware such as LoupeDeck or StreamDeck | Use Bitfocus Companion to enable OSC messages on the audio-visual hardware, or the [Stream Deck plugin bridge](streamdeck.md) for Elgato decks. |
