...
Layouts are stored in the profile companion JSON (`<profile>.json`, key `obs_overlay`) next to the profile XML. Saving the GEX profile (including Save As into another folder) writes the overlay into that same file. **Export overlay...** copies the current layout to a JSON file you choose. **Import overlay...** replaces this profile’s overlay with a JSON file.
...
### Discord / application share

A separate overlay window is composed into a **screen** share, but Discord **application** share only captures that one program. Enable **Include in app share** on Canvas and pick the game (or other) window, then share that same application in Discord. GEX parents the live overlay into that window so window capture can see it.

This is best-effort: use **windowed or borderless**. Exclusive full-screen and many DirectX/Vulkan game captures still omit a foreign overlay. If the game is not running, the overlay stays a normal window (screen share still works). OBS window capture of the GEX overlay is unchanged.
...
- Drag, resize, snap-to-grid, Shift+click multi-select. Hold the **middle mouse button** to pan the designer canvas. Hold **Shift** while resizing to keep the aspect ratio (widgets and groups). Right-click selected widgets to duplicate, **copy/paste properties** (position, size, visibility, label, appearance), delete, bring forward, send backward, group, or ungroup. A grouped (or multi-selected) set resizes together from the dashed bounding-box handles. Delete, Ctrl+D, Ctrl+G / Ctrl+Shift+G, Ctrl+Z, and arrow-key nudge still work. After a widget snaps to a guide, further arrow keys move it off the guide instead of sticking. Widgets that sit outside a smaller canvas (after a resize or leaving on-screen mode) stay visible in the dark overflow area and can be dragged back; the live overlay still only shows what is on the canvas. The **Hints** banner at the top of the canvas lists the mouse and key actions that apply to the current selection (or the canvas when nothing is selected). Uncheck **Hints** on the toolbar, or close the banner, to hide it. **Selection pane** (toolbar, off by default) lists widgets on this page: sort by **Name** or **Type**, click the eye to show/hide, the lock to freeze position, a row to select and center the canvas, and Group / Ungroup for a multi-selection.
- Inspector **Geometry** includes **Name** (selection pane), **Lock position**, and **Scale font with size**. **Visibility** (right pane) has the master **Visible** checkbox plus live conditions: mode, GEX state, physical button, vJoy button, or **keyboard/mouse**. Each condition is assigned a letter (A, B, C…). Type a boolean **Expression** such as `A AND (B OR C)` or `A XOR B` (AND, OR, XOR, NAND, NOR, XNOR, NOT, and parentheses). Leave the expression empty to require every condition (AND). **Preview** opens a Venn diagram (up to three inputs), the boolean algebra, a truth table, and a legend of what A/B/C refer to. **Boolean operators** shows AND, OR, XOR, NAND, NOR, XNOR, and NOT with gate symbols, Venn diagrams, algebra, and truth tables. Keyboard/mouse rows use the same combination picker as Map to Keyboard/Mouse Ex (**Select...** opens the popup). Incomplete rows are ignored. The live overlay hides the widget; the designer keeps a faded copy so you can still edit it. Hidden widgets also appear as a dashed outline in the designer. Appearance puts **Opacity** (percent slider) above fill color, then widget-specific fill/dot controls. A fill with **alpha 0** draws outline only (the border stays visible). **Blinking** is off until you check a trigger (Off→On, On→Off, While on, While off, and/or a GEX state). Frequency is in Hz. Permanent keeps blinking while the condition holds; Temporary blinks for the chosen duration after a trigger. While blinking, the widget swaps Off and On appearance. **Grid**, **Crosshairs**, **Axis** (N/S/E/W labels, distance, font), and **Border** (color, width, corner radius) are separate groups. Grids reach the widget border; **Fade at border** tapers them toward the edge.
...
- Inspector: size, **Rotation** (−180° to 180°, clockwise), visibility conditions, colors, fonts (family, size, bold/italic/underline/strikethrough, shadow offset and color, stroke width and color), line/dot sizes, labels (including fill and border), invert, deadzone, blinking, and binding. Drag the round handle above a selected widget to rotate it; hold Shift to snap to 15°. Selecting a **group** (or multiple widgets) shows only shared properties; edits apply to every selected widget. With **Scale font with size** on, Font size shows the size currently drawn and updates while you resize the widget.
- **Guides** (Canvas): add vertical and/or horizontal lines, set a color, drag them on the canvas, or type a percent of width/height. Moving or resizing a widget snaps its left/right/center (or top/bottom/center) to nearby guides. Nudging with the arrow keys can snap onto a guide, then a further nudge moves away instead of sticking.
...
- Layouts are per profile. Saving the GEX profile writes the overlay into `<profile>.json` (key `obs_overlay`) next to that profile XML. **Export overlay...** copies the layout to another file. **Import overlay...** loads a JSON file into the current profile.
