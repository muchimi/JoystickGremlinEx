...
class TabData:
    def __init__(self, device_guid, tab_type : TabDeviceType, device : dinput.DeviceSummary):
...
class GremlinUi(QtWidgets.QMainWindow):
    ...
    def _add_tab(self, device_guid, tab_type, index = None) -> int: 
        ...
        if not device.connected:
            ...
            if not has_mapping:
                # icon = gremlin.ui.ui_common.Icons.mappedIcon()
                # self.ui.devices.setTabIcon(position, icon)
                color = gremlin.ui.ui_common.Color().unmappedColor()
                self.ui.devices.setTabTextColor(position, color)   
    ...
    def registerWidget(self, device_guid, widget, hide = True) -> int:
        ...
        #current_index = self.ui.device_widget.currentIndex()
    ...
    def _find_tab_index(self, search_guid : str):
        ...
        for device_guid, _, _, tab_index in tab_map.values():
            if gremlin.util.compare_guid(device_guid, search_guid):
                return tab_index
        return None
