...
class StateContainerWidget(AbstractContainerWidget):
    ...
    def __init__(self, input_item : gremlin.input_item.AbstractInputItem, container : StateContainer, parent=None):  # noqa: F821
    ...
    def _create_action_ui(self):
        ...
        rb = gremlin.ui.ui_common.QDataRadioButton(
            value=self.container.required_value,
            label="On/Pressed",
            data=True,
            callbackEx=self._handle_execute_changed,
            tooltip="The container actions execute if the state exists and is on/pressed.",
        )
        widgets.append(rb)
        rb = gremlin.ui.ui_common.QDataRadioButton(
            value=not self.container.required_value,
            label="Off/Released",
            data=False,
            callbackEx=self._handle_execute_changed,
    ...
    def _create_condition_ui(self):
        if self.container.action_sets:
            widget = self._create_action_set_widget(self.container.action_sets[0], "State", ContainerViewTypes.Conditions)
