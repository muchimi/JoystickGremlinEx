...
class GremlinUi(gremlin.ui.ui_common.QRememberMainWindow):
    ...
    def _create_tabs_ui(self, activate_tab=None):
        ...
        try:
            ...
            active_device : DeviceSummary = gremlin.joystick_handling.getDevice(active_device_guid)
            ...
            if not reset:
                ...
                tab_device_list = [item for item in self._tab_device_map] # list of UUIDs
                device_list =  [gremlin.util.to_guid(dev.device_guid) for dev in sorted_devices] # convert to list of UUIDs
                special_device_list = [gremlin.util.to_guid(dev.device_guid) for dev in gremlin.joystick_handling.getSpecialDevices()]
                device_list.extend(special_device_list)


                removed_list = [id for id in tab_device_list if not id in device_list]
                if removed_list:
                    if verbose: syslog.info(f"TAB: removing [{len(removed_list)}] disconnected devices")
            ...
            for device in sorted_devices:
                ...
                if self.profile.isRemovedDevice(device.device_id):
                    if verbose: syslog.info("\tremoved by user - skipping tab")
                    continue
    ...
    def _select_input_handler_ui(self, device_guid : dinput.GUID, restore_input_type : gremlin.input_types.InputType = None, restore_input_id = None,  force_update : bool = False, force_switch = False, tab_changed = False):
        ...
        with self._change_input_lock:
            try:
                ...
                if not gremlin.util.compare_guid(device_guid, gremlin.shared_state.active_device_guid):
