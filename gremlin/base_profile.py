...
class AbstractContainer(ProfileData):
    ...
    def __init__(self, parent, node=None):
        ...
        self._virtual_button_user_enabled = (
            False  # determins if callbacks use the virtual button function - user set
        )
        self.activation_condition = ActivationCondition(
            [], ActivationRule.All
        ...
        if input_item is not None:
            ...
            self.device = gremlin.joystick_handling.device_info_from_guid(
                self.device_guid
            )
        else:
    ...
    def generate_callbacks(self, parent=None):
        ...
        if (
            ...
            callbacks.append(
                CallbackData(
                    gremlin.execution_graph.ContainerCallback(self, parent), None
                )
            )
    ...
    def _parse_action_xml(self, node, action_set, data=None):
        ...
        for child in node:
            ...
            tag = child.tag
            if config.convert_response_curve and gremlin.base_profile._is_curve_tag(
                tag
            ):
...
class Device:
    ...
    def device_guid(self, value: dinput.GUID):
        ...
        self.connected = (
            gremlin.joystick_handling.is_device_connected(value)
        ...
        if device is not None:
            for i in range(device.axis_count):
                if i >= len(device.axis_map):
                    ...
                    mode.get_data(InputType.JoystickAxis, device.axis_map[i].axis_index)
...
class InputItem:
    ...
    def hasConditions(self):
        ...
        count = tracker.getInputItemConditionCount(
            self, gremlin.shared_state.current_mode
        )
        return count > 0
        # for container in self._containers:
        ...
        if input_id is not None and self._device_guid is not None:
            if isinstance(input_id, int):
                if self._input_type == InputType.JoystickAxis:
                    ...
                    self._calibration = mgr.getCalibration(
                ...
                if isinstance(input_id, gremlin.keyboard.Key):
                    ...
                    try:
                        ...
                        self._input_name(
                            f"Unable to parse type: {type(input_id).__name__}"
                        )
    ...
    def from_xml(self, node, data=None):
        ...
        if self.input_type in (InputType.KeyboardLatched, InputType.Keyboard):
            ...
            if "id" in node.attrib and node.tag == "key":
                ...
                if "extended" in node.attrib:
                    ...
                    key = Key(
                        scan_code=scan_code, is_extended=is_extended, is_mouse=is_mouse
                    )
                    input_item.key = key
                    for child in node:
                        if child.tag == "latched":
                            latched_key = Key(
                                scan_code=safe_read(child, "id", int),
                                is_extended=read_bool(child, "extended"),
                            )
        ...
        for child in container_node:
            if child.tag in (
                ...
                "input",
                "keylatched",
            ) or gremlin.base_profile._is_curve_tag(child.tag):
                # ignore extra data
                continue
            if "type" not in child.attrib:
                syslog.error(f"XML {child.tag} is missing container 'type' attribute")
                continue
        ...
        for entry in self.containers:
            ...
            if valid:
                ...
                if gremlin.config.Configuration().verbose:
                    syslog.info(
                        f"SaveProfile: input: {self.input_type} {InputType.to_display_name(self.input_type)} input id: {self.input_id} container has no data - won't save {entry.name}"
                    )
        ...
        if not self._name and not self._device_guid:
            ...
            while current:
                ...
                if self._name and self._device_guid:
                    break
                current = current.parent
        return hash((self._device_guid, self._name, self.input_type, self.input_id))
        ...
        for mode in modes:
            ...
            for i in range(device.axis_count):
                if i >= len(device.axis_map):
                    ...
                    new_mode.get_data(
                        InputType.JoystickAxis, device.axis_map[i].axis_index
                    )
    ...
    def build_inheritance_tree(self, as_tree=False):
            ...
            if name not in nodes:
                if parent:
                    ...
                    if name in parent_modes:
                        syslog.warning(
                            f"MODE TREE: mode {name} is already used by a parent"
                        )
                        return None
        ...
        if verbose:
            for pre, fill, node in anytree.RenderTree(
                root_node, style=anytree.AsciiStyle()
            ):
    ...
    def rename_mode(self, old_mode: str, new_mode: str):
        ...
        for mode in modes:
            if mode.name == old_mode:
                if verbose:
                    syslog.info(f"PROFILE: rename [{old_mode}] to [{new_mode}]")
                mode.name = new_mode
                return True

        if verbose:
            syslog.error(
                f"PROFILE: rename [{old_mode}] to [{new_mode}] - [{old_mode}] was not found in the profile"
            )
        return False
    ...
    def list_unused_vjoy_inputs(self):
        ...
        for entry in vjoy_devices:
            ...
            for i in range(entry.axis_count):
                vjoy[entry.vjoy_id]["axis"].append(entry.axis_map[i].axis_index)
    ...
    def from_xml(self, fname, data=None):
        ...
        if root_mode_node is not None:
            ...
            if verbose:
                ...
                for pre, fill, node in anytree.RenderTree(
                    mode_tree, style=anytree.AsciiStyle()
                ):
                    syslog.info(f"{pre}{node.name}")
        ...
        for dev in devices:
            ...
            if add_device:
                ...
                # Create required mode objects
                if not mode_list:
                    mode_list = gremlin.profile.mode_list()
                for mode in mode_list:
                    if mode not in new_device.modes:
                        mode_object = Mode(new_device)
                        mode_object.name = mode
                        new_device.modes[mode] = mode_object
                        if mode in mode_node_map:
                            mode_node_map[mode].mode_object = mode_object
        ...
        if backup_max > 0 and backup and os.path.isfile(use_name):
            ...
            if not os.path.isdir(backup_path):
                try:
                    ...
                    syslog.error(
                        f"BACKUP: unable to create backup folder {backup_path}:  {err}"
                    )
            ...
            if os.path.isdir(backup_path):
                ...
                if len(backup_files) > backup_max:
                    ...
                    try:
                        ...
                        syslog.error(
                            f"BACKUP: save error: Unable to remove oldest backup profile: {oldest_file}:  {err}"
                        )
...
class Mode:
    ...
    def from_xml(self, node, data=None):
        ...
        for child in node:
            ...
            # verification.
            if item.input_type == InputType.JoystickAxis and dinput.DILL.device_exists(
                self.parent.device_guid
            ):
    ...
    def to_xml(self):
        ...
        for input_type in input_types:
            item_list = sorted(
                self.config[input_type].values(), key=lambda x: x.input_id
            )
    ...
    def load_profile_map(self):
        ...
        if os.path.isfile(fname):
            ...
            try:
                ...
                syslog.error(
                    f"PROC MAP: Unable to open profile mapping: {fname}:\n{ex}"
                )
        self._update()
