...
class OscClient():
    ...
    def start(self, server_ip = None, server_port = None):
        ...
        if self._server_ip == oi.hostIp and self._output_port == oi.hostPort:
            # loopback scenario
            self._loopback = True
            if verbose: syslog.info(f"OSC loopback client: {self._name} starting {self._server_ip} port: {self._output_port}")
        else:
            # syslog = logging.getLogger("system")
            if self._server_ip is not None and self._output_port is not None:
                self._client = UDPClient(self._server_ip, self._output_port)
                self._started = True
                if verbose: syslog.info(f"OSC client: {self._name} starting {self._server_ip} port: {self._output_port}")
            else:
    ...
    def stop(self):
        ...
        if self._started:
            ...
            if self._loopback:
                ...
                if verbose: syslog.info(f"OSC: loopback client stop: ip: {self._server_ip} port: {self._output_port}")
            else:
                self._client.stop() # stop UDP client
                self._client = None
                # syslog = logging.getLogger("system")
                if verbose: syslog.info(f"OSC: client stop: ip: {self._server_ip} port: {self._output_port}")
...
class OscInterface(QtCore.QObject):
    ...
    def _osc_message_handler(self, address, *args):
        ...
        verbose = gremlin.config.Configuration().verbose_mode_osc
        if verbose: syslog.info(f"OSC: received: {address}: {args}")
...
class OscInputItem(gremlin.base_profile.InputItem):
    ...
    def source_index(self, value : int):
        if value >= 0 and self._source_index != value:
            ...
            tracker = gremlin.ui.ui_common.DeviceWidgetTracker()
            tracker.unregisterWidget(self._device_guid, current_mode, self._input_type, self.getCompoundMessageKey(), self._guid)
            ...
            if input_item:
                ...
                if not widget:
                    if not pop_cursor:
                        gremlin.util.pushCursor()
                        pop_cursor = True
                    widget = gremlin.ui.input_item.InputItemMappingWidget(input_item = input_item, object_name=f"OSC: {input_item.display_name}")
        ...
        for input_widget in widgets:
            ...
            for widget in compare_widgets:
                ...
                if key == other_key:
                    ...
                    self._set_status(widget,"ph.shield-warning-fill", f"Input conflict detected with input [{input_widget_index + 1}]", color = icon_color)
                    self._set_status(input_widget,"ph.shield-warning-fill", f"Input conflict detected with input [{index + 1}]", color = icon_color)
