...
from gremlin.base_conditions import ActivationRule, KeyboardCondition, JoystickCondition, VJoyCondition, \
    ConditionTrackerData, InputActionCondition
import gremlin.keyboard
from ..base_conditions import ConditionTracker
...
class ActivationConditionWidget(QtWidgets.QWidget):
    ...
    def _create_ui(self):
        ...
        self.container_condition_model = ConditionModel(
            self.profile_data, self.profile_data.activation_condition
        )
    ...
    def _create_ui(self):
        ...
        if self.condition_data.input_item:
            self.key_label.setText(
                f"<b>{self.condition_data.input_item.display_name}</b>"
            )
        prefix = "dark_" if gremlin.shared_state.is_dark_theme else ""
        self.record_button_widget = ui_common.NoKeyboardPushButton(
            load_icon(f"gfx/{prefix}button_edit.png"), "Select Keys"
        )
...
class JoystickConditionWidget(AbstractConditionWidget):
    ...
    def _create_ui(self):
        ...
        self.record_button = QtWidgets.QPushButton(
            load_icon(f"gfx/{prefix}button_edit.png"), "Listen"
        )
        ...
        self.use_calibrated_input_widget.setChecked(
            self.condition_data.use_calibrated_data
        )
    ...
    def _axis_value(self):
        if self.condition_data.use_calibrated_data:
            value = gremlin.joystick_handling.get_axis(
                self.condition_data.device_guid, self.condition_data.input_id
            )
        else:
    ...
    def _hat_ui(self):
        ...
        if (
            not self.condition_data.comparison
            or self.condition_data.comparison.capitalize() not in directions
        ):
    ...
    def _input_pressed_cb(self, event):
        ...
        self.condition_data.device_name = (
            gremlin.joystick_handling.device_name_from_guid(event.device_guid)
...
class InputActionConditionWidget(AbstractConditionWidget):
    ...
    def _create_ui(self):
        ...
        self.grid_layout.addWidget(
            QtWidgets.QLabel("<b>this (virtual) button</b>"), 0, 1
        )
        self.grid_layout.addWidget(QtWidgets.QLabel("is"), 0, 2)
        self.grid_layout.addWidget(
...
class ConditionView(ui_common.AbstractView):
    ...
    condition_map = {
        "Keyboard": [KeyboardCondition, KeyboardConditionWidget],
        "Joystick": [JoystickCondition, JoystickConditionWidget],
        "vJoy": [VJoyCondition, VJoyConditionWidget],
        "Action": [InputActionCondition, InputActionConditionWidget],
    }
