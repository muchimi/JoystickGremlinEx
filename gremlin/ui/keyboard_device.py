                ...
                if child.tag in ("key"):
                    ...
                    for latched_child in child:
                        if latched_child.tag == "latched":
                            virtual_code = safe_read(
                                latched_child, "virtual-code", int, 0
                            )
                            scan_code = safe_read(latched_child, "scan-code", int, 0)
                            is_extended = safe_read(
                                latched_child, "extended", bool, False
                            )
                            ...
                            if is_mouse:
                                ...
                                #     key = gremlin.keyboard.KeyMap.find_virtual(virtual_code)
                                # else:
                                (scan_code, is_extended), _ = (
                                    gremlin.keyboard.KeyMap.translate(
                                        (scan_code, is_extended)
                                    )
                                )
                                key = gremlin.keyboard.KeyMap.find(
                                    scan_code, is_extended
                                )
...
class KeyboardDeviceTabWidget(gremlin.ui.ui_common.QSplitTabWidget):
    ...
    def __init__(self, device_profile, current_mode, parent=None):
        ...
        self.input_item_list_view = input_item.InputItemListView(
            custom_widget_handler=self._custom_widget_handler, parent=self
        )
        ...
        clear_keyboard_button = ui_common.ConfirmPushButton(
            "Clear Keys", show_callback=self._show_clear_cb
        )
    ...
    def _update_input_widget(self, input_widget, container_widget):
        ...
        if is_warning:
            ...
            icon = gremlin.util.load_icon(
                "ph.warning-light", use_qta=True, qta_color=QtGui.QColor(warning_color)
            )
