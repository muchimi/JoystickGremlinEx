...
class Device(QtCore.QAbstractListModel):
    ...
    def rowCount(self, parent: QtCore.QModelIndex = ...) -> int:
        ...
        return self._device.axis_count + self._device.button_count + self._device.hat_count
    ...
    def _convert_index(self, index: int) -> typing.Tuple[InputType, int]:
        ...
        if index < axis_count:
            return (InputType.JoystickAxis, self._device.axismap_list[index].axis_index)
...
class VJoyDevices(QtCore.QObject):
    ...
    def __init__(self, parent=None):
        ...
        self._devices = sorted(joystick_handling.vjoy_devices(), key=lambda x: x.vjoy_id)

        # Information used to determine what to show in the UI
        self._valid_types = [InputType.JoystickAxis, InputType.JoystickButton, InputType.JoystickHat]
        self._input_items = []
        self._input_data = []
    ...
    def _is_state_valid(self) -> bool:
        ...
        """
        return self._current_vjoy_index is not None and self._current_input_index is not None and self._current_input_type is not None
    ...
    def inputModel(self):
        ...
        for input_type in self._valid_types:
            for i in range(input_count[input_type](device)):
                ...
                self._input_items.append(common.input_to_ui_string(input_type, input_id))
    ...
    def _set_valid_types(self, valid_types: typing.List[str]) -> None:
        ...
        if type_list != self._valid_types:
            ...
            input_label = common.input_to_ui_string(InputType.to_enum(old_input_type), old_vjoy_id)
    ...
    def _get_vjoy_id(self) -> int:
        if not self._is_state_valid():
            raise error.GremlinError("Attempted to read from invalid VJoyDevices instance.")
    ...
    def _set_vjoy_index(self, index: int) -> None:
        if index != self._current_vjoy_index:
            if index >= len(self._devices):
                raise error.GremlinError(f"Invalid device index used device with index {index} does not exist")
    ...
    validTypes = Property("QVariantList", fget=_get_valid_types, fset=_set_valid_types, notify=validTypesChanged)

    vjoyId = Property(int, fget=_get_vjoy_id, notify=vjoyIdChanged)

    vjoyIndex = Property(int, fget=_get_vjoy_index, fset=_set_vjoy_index, notify=vjoyIndexChanged)

    inputId = Property(int, fget=_get_input_id, notify=inputIdChanged)

    inputIndex = Property(int, fget=_get_input_index, fset=_set_input_index, notify=inputIndexChanged)
...
class AbstractDeviceState(QtCore.QAbstractListModel):
    ...
    def _event_handler_impl(self, event: event_handler.Event) -> None:
        raise error.GremlinError("AbstractDeviceState._event_handler_impl not implemented")
    ...
    def _initilize_state(self) -> None:
        raise error.GremlinError("AbstractDeviceState._initialize_state not implemented")
...
class DeviceAxisState(AbstractDeviceState):
    ...
    def _initialize_state(self) -> None:
        for i in range(self._device.axis_count):
            ...
            self._state.append({"identifier": self._device.axismap_list[i].axis_index, "value": 0.0})
...
class DeviceAxisSeries(QtCore.QObject):
    ...
    def _set_guid(self, guid: str) -> None:
        ...
        for i in range(self._device.axis_count):
            self._identifier_map[self._device.axismap_list[i].axis_index] = i
            self._state.append({"identifier": self._device.axismap_list[i].axis_index, "timeSeries": []})
        self.deviceChanged.emit()
    ...
    windowSize = Property(int, fset=_set_window_size, fget=_get_window_size, notify=windowSizeChanged)
