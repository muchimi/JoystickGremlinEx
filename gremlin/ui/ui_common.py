...
class Buttons:
    ...
    def getGrabWidget(
        ...
        return Buttons._template(
            label=label, icon_source=Icons.recordIcon(), tooltip=tooltip, callback=callback, no_keyboard=no_keyboard, data=data, width=width
        )
    ...
    def getListenWidget(
        ...
        return Buttons._template(
            label=label, icon_source=Icons.listenIcon(), tooltip=tooltip, callback=callback, no_keyboard=no_keyboard, data=data, width=width
        )
    ...
    def getClearWidget(
        ...
        return Buttons._template(
            label=label, icon_source=Icons.trashIcon(), tooltip=tooltip, callback=callback, no_keyboard=no_keyboard, data=data, width=width, height=height
        )
...
class QFloatLineEditEx(QWidget):
    """double input validator with optional range limits for input axis
...
class AbstractInputSelector(QWidget):
    ...
    def __init__(
        ...
        self.grid_layout.addWidget(QWidget(), 0, 2)
    ...
    def get_selection(self):
        ...
        if device_index != -1:
            ...
            if input_index == -1:
                input_index = 0
                _input_value = self.input_item_dropdowns[device_index].itemText(input_index)
            else:
    ...
    def _initialize(self):
        raise gremlin.error.MissingImplementationError("Missing implementation of AbstractInputSelector._initialize")

    def _format_device_name(self, device):
        raise gremlin.error.MissingImplementationError("Missing implementation of AbstractInputSelector._format_device_name")

    def _device_identifier(self, device):
        raise gremlin.error.MissingImplementationError("Missing implementation of AbstractInputSelector._device_identifier")
...
class JoystickSelector(AbstractInputSelector):
    ...
    def __init__(
        ...
        :param parent the parent of this widget
        """
        super().__init__(selected_callback=callback, valid_types=valid_types, parent=parent)
...
class ModeWidget(QWidget):
    ...
    def _create_widget(self):
        ...
        self.profile_options_button_widget = QIconPushButton(
            icon=Icons.gearIcon(), tooltip="Profile Options", callback=self._handle_profile_options_cb, height=24, width=24, icon_size=18
        )
        ...
        self.mode_change = QIconPushButton(
            icon=manage_modes_icon,
...
class ConfirmBoxEx(QtWidgets.QDialog):
    def __init__(
        ...
        button_container_widget, button_container_layout = getHContainer([ok_button_widget, cancel_button_widget], left_stretch=True)
...
class QJoystickListener:
    def __init__(
        ...
        **kwargs,
        ...
        :param callback: optiona, value callback, should return values and gets the axis event  callback(event) -> values : float | list[float)
...
class QAxisRepeaterProgressbar(QProgressBar, QJoystickListener):
    ...
    def triggerUpdate(self):
        """update the widget"""
        if self._device_guid is None or self._input_id is None:
            return
...
class QButtonStateWidget(QJoystickListener, QWidget):
    ...
    def _update_value_ui(self, is_pressed: bool):
        ...
        if self._last_state_value is None or self._last_state_value != is_pressed:
            self._update_pixmap_ui(is_pressed)
...
class QOnOffStatusfWidget(QWidget):
    """on/off widget"""
...
class JoystickDeviceAxisStateWidget(QtWidgets.QGroupBox):
    ...
    def __init__(self, device: DeviceSummary, step=0.01, ui_only=False, parent=None):
        ...
        if device.axis_count:
            ...
            for input_id in device.getAxisInputIdList():
                ...
                source = EventSourceType.Virtual if device.is_virtual else EventSourceType.Any  # event comes from virtual device or directinput device
...
class HatWidget(QWidget):
    ...
    triangle = QtGui.QPolygon([QtCore.QPoint(-10, 0), QtCore.QPoint(10, 0), QtCore.QPoint(0, 15)])
...
class VigemDeviceWidget(QWidget):
...
class StateVisualizerWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        widget = getHContainer(widgets, "Button size:", widget_only=True)
        group_layout.addWidget(widget)

        self._button_widget = QWidget()
        self._button_layout = QFlowLayout(self._button_widget)
    ...
    def _cleanup_ui(self):
        ...
        for btn in self._state_buttons.values():
            gremlin.util.delete_widget(btn)
        self._state_buttons.clear()
        sd = gremlin.ui.state_device.StateData()
        sd.crud.disconnect(self._state_crud)
        config = gremlin.config.Configuration()
        config.changed.disconnect(self._config_changed)
    ...
    def _populateState_ui(self):
        ...
        for widget in self._state_buttons.values():
            widget.hide()
        ...
        if items:
            ...
            icon = Icons.warningIcon(gremlin.ui.ui_common.Color.yellowColor())
            widget = QWarningWidget("No states found.", icon=icon, tooltip="There are no states to display.")
            layout.addWidget(widget)
    ...
    def _state_changed_ui(self, state):
        ...
        if verbose:
            syslog.info(f"State Visualizer: state {state.key} changed {state.value}")
        if state.key in self._state_buttons:
            ...
            if verbose:
                syslog.warning(f"State Visualizer: state {state.key} widget not found")
    ...
    def unhook(self):
        ...
        el = gremlin.event_handler.EventListener()
        match vis_type:
            case gremlin.types.VisualizationType.State:
                el.joystick_event.disconnect(self._state_update)
...
class JoystickDeviceWidget(QWidget):
    ...
    @property
    def lineCount(self):
        if self.vis_type == gremlin.types.VisualizationType.Button:
            widget = self._widgets[0] if self._widgets else None
            if widget:
    ...
    def _create_button_hat(self):
        ...
        if self._device.button_count:
            widget = JoystickDeviceButtonStateWidget(self._device)
            self._button_widget = widget
            self._widgets.append(widget)
        if self._device.hat_count:
            widget = HatState(self._device)
            self._hat_widget = widget
            self._widgets.append(widget)
        if self._widgets:
    ...
    def buttonWidget(self):
        """Returns the button widget if available."""
        return self._button_widget

    def hatWidget(self):
        """Returns the hat widget if available."""
        return self._hat_widget
    ...
    def _create_current_axis(self):
        ...
        widget = JoystickDeviceAxisStateWidget(self._device)
...
class QUsedPushButton(QDataPushButton):
    ...
    def paintEvent(self, event):
        ...
        if self._highlight:
            ...
            gradient = QtGui.QRadialGradient(QPointF(w2, h2), min(w, h) / 2, QPointF(w2, h2))
...
class JoystickDeviceButtonStateWidget(QtWidgets.QGroupBox):
    ...
    def __init__(self, device: DeviceSummary, parent=None):
        ...
        for i in range(device.button_count):
            ...
            if not self._row_height:
                self._row_height = widget.sizeHint().height()

        self.flow_layout.lineCountChanged.connect(self._handle_line_count_changed)
        self.main_layout = self.flow_layout
    ...
    # def minimumSizeHint(self):
    #     """Provides a minimum size hint for the widget"""
    #     hint = super().minimumSizeHint()
    #     rows = max(1, self.flow_layout.lineCount)
    #     margin = 4
    #     return QtCore.QSize(hint.width(), (self._row_height + margin * 2 ) * rows )
...
class DualSlider(QWidget):
    ...
    def _width_to_logical(self, value):
        ...
        return int(round((value / groove_rect.width()) * (self._range[1] - self._range[0])))
    ...
    def mousePressEvent(self, evt):
        ...
        control = self.style().hitTestComplexControl(QtWidgets.QStyle.CC_Slider, option, position)
        lower_clicked = False
...
class QFlowLayout(QtWidgets.QLayout):
    ...
    lineCountChanged = QtCore.Signal(QWidget, int)  # fires when line count changes
    ...
    def _do_layout(self, rect, test_only):
        ...
        self.lineCountChanged.emit(self.parentWidget(), line_count)
        return y + line_height - rect.y()
...
class WidgetCacheTracker:
    ...
    def getWidget(self, key) -> tuple[QWidget, bool]:
...
class QSplitTabWidget(QDataWidget):
    ...
    def selectRegisteredWidget(self, key_or_widget):
        ...
        """
        assert key_or_widget is not None, "QSplitTabWidget: selectRegisteredWidget() key/widget cannot be NULL"
    ...
    def addLeftPanelWidget(self, widget: QWidget):
        """sets the left panel widget"""
        if widget is not None:
            self._left_container_layout.addWidget(widget)

    def setRightPanelWidget(self, widget: QWidget):
        """sets the right panel widget (only contains a single widget)"""
        index = self._right_panel_stacked_widget.indexOf(widget)
        if index != -1:
            self._right_panel_stacked_widget.setCurrentWidget(widget)

    def addRightPanelWidget(self, widget: QWidget):
    ...
    def removeRightPanelWidget(self, widget: QWidget):
...
class ResizableWindow(QMainWindow):
    ...
    def addWidget(self, widget):
        """adds a widget to the main window layout"""
        self._central_layout.addWidget(widget)

    def resizeEvent(self, event):
        # Trigger when resize starts
        if not self._is_resizing:
            self._is_resizing = True
            # bitmap image of content used while resizing (instead of redraw)
            self.resize_pixmap = QPixmap(self.size())
            self.render(self.resize_pixmap)
            # make main content invisible to speed up resize
            self._central_widget.setVisible(False)

        # Update the pixmap to the new sizing dimensions
        self.resize_pixmap = self.resize_pixmap.scaled(event.size())

        self._resize_timer.start(300)  # start or restart

        # Restart the timer or pass along to standard handler
        super().resizeEvent(event)

    def paintEvent(self, event):
        if self._is_resizing and self.resize_pixmap:
            painter = QPainter(self)
            painter.drawPixmap(0, 0, self.resize_pixmap)
            painter.end()
...
class QRememberMainWindow(ResizableWindow):
...
def getGridContainer(
    ...
    if stretch:
        ...
        layout.addWidget(QWidget(), 0, col)
...
class QJoystickRangeWidget(QWidget):
    ...
    def __init__(
        ...
        output_data_entry_widget = QWidget()
    ...
    def setOutput(self, min_value, max_value=None):
        ...
        if verbose:
            syslog.info(f"Range widget set value: {min_value:0.3f} {max_value:0.3f}  commmand: {min_cmd:0.3f} {max_cmd:0.3f}")
...
class QTypeSelectorWidget(QWidget):
...
class QAxisSourceSelector(QWidget):
    ...
    def sync(self, device_id, input_id):
        ...
        # sync device
        device = next(
            (device for device in gremlin.joystick_handling.all_joystick_devices() if device.device_id == device_id),
            None,
        )
...
class QJoystickInputWidget(QWidget):
    ...
    def _update_ui(
        ...
        has_inputs = (filtered_axis or 0) + (filtered_button or 0) + (filtered_hat or 0)
        if not has_inputs:
            warning_color = gremlin.ui.ui_common.Color.blueColor()
        ...
        if device:
            if device.axis_count:
                if filtered_axis is not None and filtered_axis > 0 and filtered_axis != device.axis_count:
                    ...
                    widget = QIconLabel(icon, f"{device.axis_count}", tooltip=tooltip)
                widgets.append(widget)
            ...
            if device.button_count:
                if filtered_button is not None and filtered_button > 0 and filtered_button != device.button_count:
                    ...
                    widget = QIconLabel(icon, f"{device.button_count}", tooltip=tooltip)
                widgets.append(widget)
            ...
            if device.hat_count:
                if filtered_hat is not None and filtered_hat > 0 and filtered_hat != device.hat_count:
                    ...
                    widget = QIconLabel(icon, f"{device.hat_count}", tooltip=tooltip)
                widgets.append(widget)
            ...
            if visible_count:
                ...
                widget = QtWidgets.QLabel(f"(<span style='color: {fcolor}; font-weight: bold;'>{visible_count}</span>/{total_count})")
                widget.setToolTip(tooltip)
                widgets.append(widget)
    ...
    def setStats(self, stats):
        ...
        if f_axis < 0:
            ...
            syslog.warning(f"STATS: used axis count is reporting negative [{f_axis}] for device [{device.name}]")
            f_axis = 0
        if f_button < 0:
            if not device:
                device = gremlin.joystick_handling.getDevice(stats.device_guid)
            syslog.warning(f"STATS: used axis count is reporting negative [{f_button}] for device [{device.name}]")
            f_button = 0
        if f_hat < 0:
            if not device:
                device = gremlin.joystick_handling.getDevice(stats.device_guid)
            syslog.warning(f"STATS: used axis count is reporting negative [{f_hat}] for device [{device.name}]")
            f_hat = 0
...
class QInputLockWidget(QWidget):
...
class QCollapsible(QFrame):
    ...
    def eventFilter(self, a0: QObject, a1: QEvent) -> bool:
        ...
        if a1.type() == QEvent.Type.Resize and self.isExpanded() and not self._is_animating:
...
class QJoystickSelectorWidget(QWidget):
...
class QJoystickSelectorDialog(QShowAtCursorDialog):
    def __init__(
        ...
        cancel_button_widget = QtWidgets.QPushButton("Cancel")
        cancel_button_widget.clicked.connect(self._close_cb)
        button_container_widget, _ = getHContainer([listen_widget, "||", ok_button_widget, cancel_button_widget])
...
class QProcessSelectorWidget(QWidget):
...
class QSendModeSelector(QWidget):
    ...
    def __init__(
        ...
        self._selector_widget = QDataComboBox(source=items, value=value, callback=self._handle_mode_changed)
...
class RemoteClientWidget(QWidget):
...
class TargetProcessWidget(QWidget):
    ...
    def _handle_select_executable(self, widget):
        """opens the process executable"""
        fname, _ = QtWidgets.QFileDialog.getOpenFileName(None, "Process", self.config.process, "Executable files (*.exe)")
        if fname and os.path.isfile(fname):
...
class WidgetManager(QtWidgets.QDialog):
    ...
    def getWidget(self, key, new_parent: QtWidgets.QLayout = None) -> QWidget:
        """gets the widget"""
...
class QArrowPairWidget(QWidget):
...
class ResizingStackedWidget(QtWidgets.QStackedWidget):
    ...
    def update_geometry(self):
        ...
        self.updateGeometry()
        self.heightChanged.emit(self.sizeHint().height())
