...
class AbstractView(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        self._has_changes = False  # true if model has changes
        self._hash_key = None  # cache for the hash key
    ...
    def popSuspended(self, reset=False, emit=True):
        ...
        if self._redraw_suspended_stack == 0 and self._redraw_pending:
            self._has_changes = False  # indicate any changes are handled
    ...
    def _handle_model_changed(self, data=None, force=False):
        ...
        if self._has_changes:
            # we already know we have changes
            return
    ...
    def resetModelChanged(self):
        """Resets the model changed status."""
        self.model.resetChanges()
        self._has_changes = False
...
class InputItem(gremlin.base_classes.AbstractInputItem):
    ...
    fieldChanged = QtCore.Signal(str, object)  # fires when a field changes (field text, self)
    ...
    def __init__(
        ...
        if isinstance(mode_node, str):
            ...
            assert mode_node is not None, "mode_node could not be resolved from the provided name"

        super().__init__(mode_node, device_guid=device_guid, input_type=input_type)
    ...
    def description(self, value):
        if not self._description_readonly:
            ...
            self.fieldChanged.emit("description", self)
    ...
    def input_name(self) -> str:
        ...
        if self._input_name_handler is not None:
            input_name = self._input_name_handler(self)
            if self._input_name != input_name:
                self._input_name = input_name
                self.fieldChanged.emit("input_name", self)
            return input_name
        return self._input_name
    ...
    def selected(self, value: bool):
        if self._selected != value:
            ...
            self.fieldChanged.emit("selected", self)
    ...
    def enabled(self, value: bool):
        if value != self.enabled:
            self._enabled = value
            self.fieldChanged.emit("enabled", self)
            el = gremlin.event_handler.EventListener()
    ...
    def is_axis(self, value: bool):
        if self._is_axis != value:
            self._is_axis = value
            self.fieldChanged.emit("is_axis", self)
            self.fieldChanged.emit("is_button", self)
        ...
        match self._input_type:
            case InputType.JoystickAxis:
                ...
                if not device:
                    ...
                    syslog.error(
                        f"INTERNAL ERROR: Unable to retrieve device axis name for device: {self.device_guid}  axis [{self._input_id}] - the device is not found in the current device list"
                    )
                else:
...
class InputItemWidget(gremlin.ui.ui_common.QBoxFrame):
    ...
    def __init__(
        ...
        mapping_changed_callback: Callable = None,
        ...
        :param mapping_changed_callback: Optional callback for mapping changes
        ...
        self._mapping_changed_callbacks = []
        if mapping_changed_callback:
            self._mapping_changed_callbacks.append(mapping_changed_callback)  # callback to use when a specific widget index must be updated
        ...
        self._title_callback = title_callback  # callback to get the title of the input item
        ...
        self._not_configured_widget = gremlin.ui.ui_common.QIconLabel(
            gremlin.ui.ui_common.Icons.yellowWarningIcon(), text="Not configured", tooltip="Not configured"
        )
        self._content_layout.addWidget(self._not_configured_widget)
        self._not_configured_widget.setVisible(False)
        ...
        # debug content
        self._debug_display_name = QtWidgets.QLabel(f"Display Name: {self._input_item._display_name}")
        self._debug_display_description = QtWidgets.QLabel(f"Description: {self._input_item.description}")
        widgets = [self._debug_display_name, self._debug_display_description]
        self._debug_container = gremlin.ui.ui_common.getVContainer(widgets, widget_only=True)
        self.main_layout.addWidget(self._debug_container)
        ...
        self._context_menu.addAction("Remove unused actions...", self._handle_remove_unused_actions)
    ...
    def setDebugEnabled(self, enabled: bool):
        """enables or disabled the debug information in the input widget visualizer"""
        self._debug_layout = enabled
        self._update_debug()

    def _update_debug(self):
        gremlin.util.InvokeUiMethod(self._update_debug_ui)

    def _update_debug_ui(self):
        # Implement the UI update logic for debug information here
        self._debug_display_name.setText(f"Display Name: {self._input_item._display_name if self._input_item else 'N/A'}")
        self._debug_display_description.setText(f"Description: {self._input_item.description if self._input_item else 'N/A'}")
        self._debug_container.setVisible(self._debug_layout)

    def _update_title(self):
        gremlin.util.InvokeUiMethod(self._update_title_ui)

    def _update_title_ui(self):
    ...
    def _handle_input_item_field_changed(self, field_name, input_item):
        ...
        match field_name:
            case "selected":
                ...
                self.setSelected(input_item.selected)
            ...
            case "description":
                ...
                self.setInputDescription(input_item.description)
            case "input_id":
                # handle input id change
                self._update_input_id()

            case _:
                # handle other changes
                pass

    def _update_input_id(self):
        gremlin.util.InvokeUiMethod(self._update_input_id_ui)

    def _update_input_id_ui(self):
        """updates the widget when the input id changes"""
        self._update_display_name_ui()
        self._update_axis_icons_ui()
        self._update_title_ui()

    def _update_display_name(self):
        gremlin.util.InvokeUiMethod(self._update_display_name_ui)

    def _update_display_name_ui(self):
        display_text = None
    ...
    def _handle_remove_unused_actions(self):
        ...
        for container in input_item.containers:
            ...
            for action_set in container.action_sets:
                ...
                for action in action_set:
                    if not action.hasOutput():
                        remove_list.append((action_set, action))

        if remove_list:
            result = gremlin.ui.ui_common.ConfirmBox(f"Remove [{len(remove_list)}] unused actions from {input_item.display_name}?")
            if result:
                input_item.containers.pushSuspend()
                for action_set, action in remove_list:
                    action_set.removeAction(action)
                input_item.containers.popSuspend()
    ...
    def addWidget(self, widget):
        """adds a custom widget to the container"""
    ...
    def _update_repeater(self):
        ...
        # map where the vents come from
        match self.input_item.input_type:
            case InputType.Midi:
                ...
                if input_item.is_axis:
                    ...
                    if not self.axis_repeater_widget or self._repeater_container_widget.widget() is None:
                        ...
                        widget = gremlin.ui.ui_common.QAxisRepeaterProgressbar(
                            ...
                            state_change_callback=self._handle_axis_state_change_callback,
                    ...
                    if not self.button_repeater_widget:  # and input_type in (InputType.JoystickButton, InputType.JoystickHat):
                        widget = gremlin.ui.ui_common.QButtonStateWidget(
                            input_item=self._input_item,
                            callback=self._get_state_callback,
                            state_change_callback=self._handle_button_state_change_callback,
    ...
    def _handle_axis_state_change_callback(self, value: float):
        config = gremlin.config.Configuration()
        if not self.selected and config.highlight_enabled and config.highlight_input_axis:
            self.selected = True
            self.trigger()
    ...
    def _mapping_changed_cb_ui(self, input_item: InputItem):
        ...
        if input_item == self._input_item:
            ...
            # refresh the mapping container
            self.updateMappingContent()
    ...
    @property
    def index(self):
        """assigned index 0 based index (internal use)"""
        return self._index
    ...
    def clear(self):
        # clears the display entries
        self.setTitle("")
        self.setInputDescription(None)
        self.setInputDescriptionIcon(None)
        self.setStatus("", None)
        self.setDescription(None)
        self.setComment(None, None)

    def setConfigured(self, value: bool):
        if value:
            self._not_configured_widget.setVisible(False)
        else:
            self.clear()
            self._not_configured_widget.setVisible(True)
    ...
    def addUpdateCallback(self, callback: Callable):
        ...
        if callback not in self._mapping_changed_callbacks:
            self._mapping_changed_callbacks.append(callback)

    def removeUpdateCallback(self, callback: Callable):
        """removes a previously added update callback"""
        if callback in self._mapping_changed_callbacks:
            self._mapping_changed_callbacks.remove(callback)

    def updateMappingContent(self, operation: str = "update"):
        """indicates the mapping content has changed and should be updated"""
        if self._mapping_changed_callbacks:
            for callback in self._mapping_changed_callbacks:
                callback(self, operation)
    ...
    def _handle_action_icon_clicked(self, widget):
        ...
        input_item: InputItem
        action: AbstractAction
        input_item, action = widget.data

        if not self.selected:
            self.selected = True  # first select it, which updates the mapping

        mapping_widget = input_item.getMappingWidget()

        if mapping_widget is None:
            # not set yet, find it
            device_guid = input_item.device_guid
            base_widget: BaseDeviceTabWidget = gremlin.shared_state.ui.getRegisteredWidget(device_guid)
            assert base_widget is not None, f"Base widget for device {device_guid} not found"
            mapping_widget = base_widget.getInputItemMappingWidget(input_item)
            assert mapping_widget is not None, f"Mapping widget for input item {input_item.display_name} not found"
            input_item.setMappingWidget(mapping_widget)
        if mapping_widget:
            mapping_widget.ensureActionVisible(action)
...
class InputItemListModel(AbstractCallbackModel):
    ...
    def __init__(
        ...
        custom_change_handler: Callable[[object], None] = None,
        ...
        super().__init__(
            ...
            item_changed_callback=custom_change_handler,
        )
    ...
    def indexOfInputItem(self, input_item: InputItem):
        """gets the zero based index of the input item in the model (unfiltered)"""
...
class InputItemListView(AbstractView):
    ...
    def __init__(
        ...
        mapping_changed_handler: Callable[[InputItemWidget, str]] = None,
        ...
        self.mapping_changed_handler = mapping_changed_handler
        self._selection_change_callbacks = []
        self._model_hash = None  # model hash key
        ...
        self._redraw_selected_index = -1  # index of the item to select on redraw (ignored if -1)
    ...
    def removeRow(self, index):
        ...
        if input_item:
            ...
            self._redraw_selected_index = new_index
            self.redraw(True)
    ...
    def widget(self, index):
        ...
        # if __debug__:
        #     if index != -1:
        #         syslog.warning(f"InputItemListView: widget not found for index {index}")
        #         syslog.warning("Valid indices:")
        #         for valid_index in self._widget_map.keys():
        #             syslog.warning(f"  {valid_index}  {self._widget_map[valid_index]}")
        #         pass
        return None
                        ...
                        if not widget:
                            ...
                            if self.custom_widget_handler:
                                ...
                                widget = InputItemWidget(
                                    ...
                                    mapping_changed_callback=self._handle_mapping_changed,
                                )
    ...
    def _redraw_ui(self, force: bool = False):
        ...
        if self._redraw_lock:
            if verbose and self._redraw_force:
        ...
        try:
            ...
            if self._redraw_selected_index != -1 and self._redraw_selected_index < model_count:
                self.setCurrentIndex(self._redraw_selected_index)
    ...
    def _select_item_ui(self, index, emit=True, force=False, user_selected=False):
        ...
        if index == -1:
            ...
            if widget:
                index = widget.index  # index is 1 based, widget index is 0 based
        ...
        if self._current_index == -1 or self._current_index != index:
            ...
            assert widget is not None, f"widget [{index}] not found in list - check index"
...
class AbstractContainer(BaseProfileData, ConditionContainer):
    ...
    interaction_types: list[
        Interactions
    ] = []  # list of allowed interactions for derived containers, if empty, the container does not allow interactions on actions sets

    def __init__(
        self,
        parent: InputItem | "gremlin.profile_graph.ProfileContainerNode",
        node=None,
    ...
    def dumpActionSet(self, action_set: ActionSet, label=None, indent=""):
        ...
        if label:
            ...
            syslog.info(
                f"{indent}Action set: description: [{action_set.description if action_set.description is not None else 'N/A'}] model description: [{action_set.model_description if action_set.model_description is not None else 'N/A'}]"
            )
    ...
    def has_conditions(self):
        ...
        return self._activation_condition is not None and len(self._activation_condition.conditions) > 0
    ...
    def condition_count(self) -> int:
        """gets the count of container conditions currently defined at the container level"""
        return len(self._conditions)

    @property
    def action_condition_count(self) -> int:
        """gets the count of action conditions currently defined at the action level"""
        count = 0
        for action_set in self.action_sets:
            if action_set:
                count += sum(action.condition_count for action in action_set)
        return count

    @property
    def conditions(self):
        """gets the list of conditions model for this container"""
    ...
    @property
    def display_name(self) -> str:
        return f"[{self.input_display_name}] action set count: [{self.action_sets.count()}] description: [{self.description if self.description else 'n/a'}] model description: [{self.action_sets.modelDescription if self.action_sets.modelDescription is not None else 'n/a'}]"
    ...
    def from_xml(self, node, data=None, extra_data=None):
        ...
        if self.actionsetParseCallback is not None:
            ...
            if not self.custom_action_sets:
    ...
    def _parse_activation_condition_xml(self, node, data, extra_data=None):
        ...
        self.activation_condition = BaseActivationCondition(self, ActivationRule.All)
        input_item = data
...
class AbstractAction(BaseProfileData):
    """Base class for all actions that can be encoded via the XML and UI system."""
    ...
    def __init__(self, parent, extra_data: dict = None):
        ...
        self._conditions = ConditionModel(self)
        self.activation_condition = BaseActivationCondition(self, ActivationRule.All)  # stores the conditions attached to that action
    ...
    @property
    def conditions(self) -> ConditionModel:
        return self._conditions

    @property
    def condition_count(self) -> int:
        """gets the count of conditions currently defined for this action"""
        return len(self._conditions)
    ...
    def display_name(self):
        """display name for this action - override in subclasses if needed"""
        return "Action"
    ...
    def hasOutput(self):
        """Returns whether or not the action has output"""
        return True  # assume the action has output by default - derived actions can change this as needed
...
class BaseAbstractCondition:
    ...
    @property
    def target(self) -> AbstractContainer | AbstractAction:
        return self._target
    ...
    @property
    def isContainerCondition(self) -> bool:
        return isinstance(self._target, AbstractContainer)
...
class BaseActivationCondition(gremlin.base_classes.BaseCallbacks):
    ...
    def from_xml(self, node, data=None, extra_data=None):
        ...
        for cond_node in condition_nodes:
            ...
            condition = self.condition_lookup[condition_type](target=container)
...
class ActionSetsView(AbstractView):
    ...
    def _create_action_set_widget(
        ...
        :param action_set: data of the actions which form the action set
        ...
        assert isinstance(action_set, ActionSet), "invalid action set model provided"
        ...
        verbose = gremlin.config.Configuration().verbose_mode_ui_level(1)
        if verbose:
            syslog.info(f"ContainerWidget: created action set model: {action_set.debug_name}")
    ...
    def getActionSetWidget(self, action_set: ActionSet) -> ActionSetView:
        """returns the action set view widget for the given action set"""
    ...
    def _create_ui(self):
        ...
        for index, action_set in enumerate(self.model):
            widgets = [gremlin.ui.ui_common.QHorizontalLine()]
            if action_set.description:
                widgets.append(QtWidgets.QLabel(action_set.description))

            widget = gremlin.ui.ui_common.getVContainer(widgets, widget_only=True)
            self.main_layout.addWidget(widget)
            self._widgets.append(widget)

            widget = self._create_action_set_widget(action_set=action_set, view_type=self._view_type)
...
class ActionSetView(AbstractView):
    ...
    def getActionWidget(self, action: AbstractAction):
        """Returns the widget associated with the given action ID."""
        action_id = action.id
        return self._widget_map.get(action_id, None)
    ...
    def _redraw_ui(self, force=False):
        ...
        hash_key = self.model.hashKey()
        force = force or hash_key != self._last_action_hash

        if not self._drawn_once or force or self.modelChanged() or widget_count != model_count:
            ...
            self._drawn_once = True
            self._show_content()
            self._last_action_hash = hash_key
            return  # done
...
class AbstractContainerWidget(QtWidgets.QDockWidget):
    ...
    def __init__(self, input_item: InputItem, container: AbstractContainer, parent=None, view=False):
        ...
        self.collapsible_widget.setContentWidget(
            self._abstract_container_content_widget, own=False
        )  # content of the collapsible widget holds the container mappings
    ...
    def _create_container_id_ui(self):
        ...
        id_widget = gremlin.ui.ui_common.QDataLabel(
            f"Container type: [{self._container.__class__.__name__ if self._container else 'N/A'}]: [{gremlin.util.normalize_guid(self._container.id) if self._container else 'N/A'}]"
        )
    ...
    def _create_action_tab(self):
        ...
        if self._use_view:
            ...
            for action_set in self.container.action_sets:
                self._widget_map[action_set] = self._action_view.getActionSetWidget(action_set)
    ...
    def _get_widget_index(self, widget):
        ...
        """
        for i, entry in enumerate(self._widget_map.values()):
            if entry == widget:
                return i
        return -1
    ...
    def _update_condition_ui(self, container=None):
...
class InteractableActionWrapper(QtWidgets.QWidget):
    def __init__(self, action_widget, action_interact_callback: Callable = None, index: int = 0, max_index: int = 0, parent=None):
        ...
        widgets = [("", 100), self.interact_widget]
...
class ConditionActionWrapperWidget(AbstractActionWrapper):
    ...
    def __init__(self, action_widget, parent=None):
        ...
        if action.activation_condition is None:
...
class ContainerView(AbstractView):
    ...
    def _ensure_action_visible_ui(self, action: AbstractAction):
        ...
        assert container.id in self._widget_map, f"Container widget for id {container.id} not found"
        container_widget: AbstractContainerWidget = self._widget_map.get(container.id)
        action_set_view: ActionSetView
        for action_set in container.action_sets:
            if action in action_set:
                action_set_view = container_widget.getActionSetView(action_set)
                widget = action_set_view.getActionWidget(action)
                if widget:
                    self._scroll_to_item(widget)
                break
...
class InputItemMappingWidget(QtWidgets.QWidget):
    ...
    def create_ui(self):
        ...
        self._stacked_widget.addWidget(container_widget)  # index 2
    ...
    def ensureActionVisible(self, action: AbstractAction):
        """ensures the given action is visible in the UI"""
        input_item = self._input_item
        syslog.info(f"ensure action visible: input item [{input_item.display_name}] - mapping widget id: {self.id}")
        container_view = self.getContainerView()
        if not container_view:
            self.redraw(force=True)  # force an update
            container_view = self.getContainerView()

        assert container_view is not None, "Container view is not set"
        container_view.ensureActionVisible(action)
    ...
    def _redraw_ui(self, force=False):
        ...
        # look for changes in the model

        container_hash = self._input_item.containers.hashKey()
        force = container_hash != self._last_container_hash
        ...
        if force or not self._drawn_once or self._container_view is None:
            if self._input_item is not None:
                ...
                self.create_ui()
                self._last_container_hash = container_hash
        ...
        if isinstance(oc, ObjectEncoder):
            ...
            if oc.encoder_type == EncoderType.ActivationCondition:
                ...
                if node.tag == "activation-condition":
                    ...
                    activation_condition = BaseActivationCondition(self, ActivationRule.All)
...
class ConditionView(AbstractView):
    ...
    def __init__(self, container: AbstractContainer | AbstractAction, title: str = "Conditions", parent=None):
        ...
        assert isinstance(container, AbstractContainer | AbstractAction), "invalid container"
        super().__init__(model=container.conditions, parent=parent)
        ...
        items = ["Keyboard Condition", "Joystick Condition", "vJoy Condition", "Action Condition", "State Condition", "Mode Condition"]
        self.condition_selector = gremlin.ui.ui_common.QDataComboBox(source=items, value=last_selector, callback=self._handle_condition_selector_changed)

        self.condition_selector.currentIndexChanged.connect(self._handle_condition_selector_changed)
    ...
    def _copy_condition(self):
        helper = ConditionHelper()
        helper.copy_condition(self._model)

    @QtCore.Slot()
    def _paste_condition(self):
        clipboard = gremlin.clipboard.Clipboard()
        helper = ConditionHelper()
        helper.paste_condition(self._model, clipboard.data)

    @QtCore.Slot()
    def _handle_condition_selector_changed(self, data):
        config = gremlin.config.Configuration()
    ...
    def _rule_changed_cb(self, text):
        ...
        """
        self._model.rule = ConditionView.rules_map[text]

    def _model_changed(self):
        """Updates the view when the model changes."""
        self.rule_selector.setCurrentText(ConditionView.rules_map[self._model.rule])
        self.redraw()
...
class BaseDeviceTabWidget(gremlin.ui.ui_common.QSplitTabWidget):
    ...
    def pushSuspended(self):
        ...
        if self._input_item_list_view is not None:
            self._input_item_list_view.pushSuspended()

    def popSuspended(self, reset: bool = False):
        """pops the input item list view from a suspended state"""
        if self._input_item_list_view is not None:
            self._input_item_list_view.popSuspended(reset)
    ...
    def setInputItemListModel(self, model: InputItemListModel):
        ...
        # trap input item add/remove events
        if self._input_item_list_model is not None:
            self._input_item_list_model.addOnItemChangedCallback(self.onInputItemChange)

    def onInputItemChange(self, model: AbstractCallbackModel, index: int, old_value: object, new_value: object, operation: str):
        """callback for when an input item changes"""
        if operation in ("add", "update"):
            self.updateMappingContent(new_value)
    ...
    def _handle_mapping_changed(self, widget: InputItemWidget, operation: str):
        """called when the input item widget reports a mapping change for its associated input item"""
        if operation == "update":
            self.updateMappingContent(widget.input_item)
