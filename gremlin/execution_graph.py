...
class ExecutionGraphNodeType(Enum):
    ...
    ActivationCondition = (
        auto()
    )  # node is an activation condition (has a functor to evaluate)
...
class ExecutionGraphNode(anytree.NodeMixin):
    ...
    def __init__(self, node_type: ExecutionGraphNodeType):
        ...
        self.container: gremlin.base_profile.AbstractContainer = (
            None  # holds the container object
        )
        self.condition: gremlin.actions.ActivationCondition = (
            None  # holds the condition if a condition node
        )
        ...
        self.action_set: list[
            gremlin.base_profile.AbstractAction
        ] = []  # list of actions
        self.mode: str = None
        ...
        self.description = ""
        self.has_actions = (
            False  # assume the node has no child action somewhere down the tree
        )
    ...
    def __str__(self):
        ...
        match self.nodeType:
            ...
            case ExecutionGraphNodeType.Container:
                ...
                condition: gremlin.base_profile.AbstractCondition = (
...
class ExecutionContext:
    ...
    def reset(self, force_rebuild=False):
        ...
        rebuild = (
            force_rebuild or self._last_hash is None or self._last_hash != profile_hash
        )
    ...
    def searchModeTree(self, mode: str) -> ExecutionModeNode:
        ...
        if nodes:
            if len(nodes) > 1:
                syslog.warning(
                    f"CONTEXT: More than one mode named {mode} detected - returning the first one"
                )
                for node in nodes:
    ...
    def getModeNames(self, as_tuple=False, include_current=True) -> list:
        ...
        if as_tuple:
            ...
            return [
                (node.mode, node.display)
                for node in anytree.PreOrderIter(mode_tree)
                if node.mode and node.mode != current_mode
            ]
    ...
    def getNode(self, id):
        ...
        node = next(
            (node for node in anytree.PreOrderIter(self.root) if node.id == id), None
        )
        return node
        ...
        if node:
            ...
            if verbose:
                if activation_conditions:
                    syslog.info(
                        f"(node {node.id} type: {node.nodeType.name}) Activation condition functors for node {node.description}  :"
                    )
                    for n in activation_conditions:
                        syslog.info(
                            f"\t(node {node.id} type: {node.nodeType.name}) Functor node: {n.description}"
                        )
    ...
    def getModes(self) -> list:
        ...
        return [
            ...
            if node.nodeType == ExecutionGraphNodeType.Mode and node.mode
        ]
        ...
        if node:
            ...
            while not callback_list and node is not None:
                ...
                if not mode:
                    ...
                    break
                if verbose:
                    syslog.info(f"CONTEXT: Search callbacks for mode : {mode} {key}")
                ...
                node = node.parent
                if verbose:
                    syslog.info(f"\tNot found, using parent node: {node.name}")
    ...
    def deviceHasMappings(self, device_guid):
        ...
        if self.root:
            for node in anytree.PreOrderIter(self.root):
                if (
                    node.nodeType == ExecutionGraphNodeType.Device
                    and node.device.device_id == device_guid
                ):
                    # found the device
                    if anytree.find(
                        node,
                        filter_=lambda n: n.nodeType
    ...
    def find(self, item):
        ...
        for node in anytree.PreOrderIter(self.root):
            if (
                node.container == item
                or node.functor == item
                or node.action_set == item
                or item in node.functors
            ):
                return node
        return None
        ...
        for node in anytree.PreOrderIter(self.root):
            if node.nodeType == ExecutionGraphNodeType.InputItem:
                ...
                if (
                    ...
                    and input_item.input_id == input_id
                ):
                    return input_item
    ...
    def dumpActive(self):
        ...
        if self.root:
            for pre, fill, node in anytree.RenderTree(
                self.root, style=anytree.AsciiStyle()
            ):
                if anytree.search.findall_by_attr(
                    node, ExecutionGraphNodeType.Action, "nodeType"
                ):
                    syslog.info(f"{pre}{str(node)}")
    ...
    def dumpModeTree(self):
        ...
        if self.modeTree:
            for pre, fill, node in anytree.RenderTree(
                self.modeTree, style=anytree.AsciiStyle()
            ):
    ...
    def _create_activation_condition(
        self, activation_condition, target, is_container_condition=False
    ):
    ...
    def _get_condition_node(self, owner, parent):
        ...
        match rule:
            case gremlin.actions.ActivationRule.Any:
                ...
                condition_nexus = ExecutionGraphNode(
        ...
        if conditions:
            for condition in conditions:
                sub_node = ExecutionGraphNode(
                    ExecutionGraphNodeType.ActivationCondition
                )
                sub_node.condition = condition
                sub_node.description = (
                    f"(sub) ActivationCondition node: {str(condition)}"
                )
    ...
    def _register_condition(self, parent_node, node):
        ...
        syslog.info(
            f"Register condition: {node_id} {parent_node.description} -> {node.description}"
        )
            ...
            match node.nodeType:
                ...
                case ExecutionGraphNodeType.ActivationConditionNexus:
                    ...
                    condition_nodes = [
                        ...
                        if n.nodeType == ExecutionGraphNodeType.ActivationCondition
                    ]
                    other_nodes = [
                        n
                        for n in node.children
                    ...
                    syslog.info(
                        f"{logTabs}Added {len(any_functors)} condition functors"
                    )
                ...
                case ExecutionGraphNodeType.ActivationCondition:
                    if node.condition:
                        ...
                        if condition and container:
                            if isinstance(
                                condition, gremlin.base_conditions.ActivationCondition
                            ):
                                ...
                                condition, gremlin.base_conditions.AbstractCondition
                            ):
                                syslog.info(
                                    f"{logTabs}\tadding functor for condition: {str(condition)}"
                                )
                                functor = self._convert_condition(condition)
            ...
            if n.nodeType in (
                ...
                if n.condition:
                    ...
                    if condition and container:
                        ...
                        syslog.info(
                            f"{logtabs}\tAdding activation container condition: {str(condition)}"
                        )
        ...
        if extra_inputs:
            ...
            for device_guid, input_type, input_id in extra_inputs:
                event = gremlin.event_handler.Event(
                    event_type=input_type, device_guid=device_guid, identifier=input_id
                )
    ...
    def _build_execution_tree(self):
        ...
        for device in profile.devices.values():
            ...
            for mode in device.modes.values():
                if mode.name not in mode_nodes:
                    syslog.error(
                        f"Execution Tree: error: mode: {mode.name} is not found in the device node: {device_node.name}"
                    )
                    continue
                ...
                for input_items in mode.config.values():
                    for input_item in input_items.values():
                        ...
                        input_node = ExecutionGraphNode(
                            ExecutionGraphNodeType.InputItem
                        )
                        ...
                        input_container_group = ExecutionGraphNode(
                            ExecutionGraphNodeType.Group
                        )
                        ...
                        for container in input_item.containers:
                            ...
                            assert isinstance(
                                container, gremlin.base_profile.AbstractContainer
                            ), f"invalid node type: {container.__class__.__name__} encountered"

                            # container.refresh_conditions() # refresh any conditions for this container
                            container_node = ExecutionGraphNode(
                                ExecutionGraphNodeType.Container
                            )
                            ...
                            # container condition
                            condition_node = self._get_condition_node(
                            ...
                            for action_set in container.action_sets:
                                for action in action_set:
                                    action_condition_node = self._get_condition_node(
                                        action, condition_node
                                    )

                                    # action node
                                    action_node = ExecutionGraphNode(
                                        ExecutionGraphNodeType.Action
                                    )
                                    ...
                                    functor = self._get_action_functor(
                                        action, action_node
                                    )
                                    action_node.exec_functors = functor
                                    action_node.description = (
                                        f"Action node: {str(action)}]"
                                    )
                                    ...
                                    if action.name == "Gated Axis":
                                        ...
                                        for gate_info in gates:
                                            gate_node = ExecutionGraphNode(
                                                ExecutionGraphNodeType.Gate
                                            )
                                            gate_node.description = (
                                                f"Gate {gate_info.to_display()}"
                                            )
                                            ...
                                            for (
                                                condition_type,
                                                item_data,
                                            ) in gate_info.item_data_map.items():
                                                gate_condition_node = ExecutionGraphNode(
                                                    ExecutionGraphNodeType.GatedAxisCondition
                                                )
                                                gate_condition_node.exec_functors = gremlin.gated_handler.GatedAxisGateCondition(
                                                    gate_data, gate_info, condition_type
                                                )
                                                gate_condition_node.parent = gate_node

                                                group_node = ExecutionGraphNode(
                                                    ExecutionGraphNodeType.Group
                                                )
                                                group_node.parent = gate_condition_node

                                                for container in item_data.containers:
                                                    gate_container_node = ExecutionGraphNode(
                                                        ExecutionGraphNodeType.Container
                                                    )
                                                    gate_container_node.id = (
                                                        container.id
                                                    )
                                                    gate_container_node.container = (
                                                        container
                                                    )
                                                    ...
                                                    gate_activation_condition_node = (
                                                        self._get_condition_node(
                                                            container,
                                                            gate_container_node,
                                                        )
                                                    )
                                                    ...
                                                    for (
                                                        action_set
                                                    ) in container.action_sets:
                                                        for gate_action in action_set:
                                                            gate_action_condition_node = self._get_condition_node(
                                                                gate_action,
                                                                gate_activation_condition_node,
                                                            )
                                                            gate_action_node = ExecutionGraphNode(
                                                                ExecutionGraphNodeType.Action
                                                            )
                                                            gate_action_node.id = (
                                                                gate_action.id
                                        ...
                                        for range_info in gate_data.getUsedRanges():
                                            range_node = ExecutionGraphNode(
                                                ExecutionGraphNodeType.Range
                                            )
                                            range_node.parent = action_node
                                            range_node.description = (
                                                f"Range {range_info.to_display()}"
                                            )

                                            for (
                                                condition_type,
                                                item_data,
                                            ) in range_info.item_data_map.items():
                                                range_condition_node = ExecutionGraphNode(
                                                    ExecutionGraphNodeType.GatedAxisCondition
                                                )
                                                range_condition_node.exec_functors = gremlin.gated_handler.GatedAxisRangeCondition(
                                                    gate_data,
                                                    range_info,
                                                    condition_type,
                                                )
                                                ...
                                                for index, container in enumerate(
                                                    item_data.containers
                                                ):
                                                    range_container_node = ExecutionGraphNode(
                                                        ExecutionGraphNodeType.Container
                                                    )
                                                    range_container_node.id = (
                                                        container.id
                                                    )
                                                    range_container_node.container = (
                                                        container
                                                    )
                                                    ...
                                                    range_activation_condition_node = (
                                                        self._get_condition_node(
                                                            container, group_node
                                                        )
                                                    )
                                                    ...
                                                    for (
                                                        ...
                                                        for range_action in action_set:
                                                            range_action_condition_node = self._get_condition_node(
                                                                range_action,
                                                                range_container_node,
                                                            )

                                                            range_action_node = ExecutionGraphNode(
                                                                ExecutionGraphNodeType.Action
                                                            )
                                                            range_action_node.id = (
                                                                range_action.id
                                                            )
                                                            range_action_node.parent = range_action_condition_node  # range action is owned by its condition
                                                            range_action_node.action = (
                                                                range_action
                                                            )
                                                            functor = self._get_action_functor(
                                                                range_action,
        ...
        action_nodes = anytree.findall_by_attr(
            self.root, value=ExecutionGraphNodeType.Action, name="nodeType"
        )
                ...
                for key in callbacks[device_guid][mode]:
                    ...
                    for callback, _ in callbacks[device_guid][mode][key]:
                        if hasattr(callback, "execution_graph"):
                            container_graph: ContainerExecutionGraph = (
                                callback.execution_graph
                            )
                            ...
                            node = next(
                                (
                                    n
                                    for n in anytree.PreOrderIter(self.root)
                                    if n.nodeType == ExecutionGraphNodeType.Container
                                    and n.id == id
                                ),
                                None,
                            )
    ...
    def registerNode(self, node):
        ...
        if verbose:
            logtabs = gremlin.shared_state.logTabs()
            syslog.info(
                f"{logtabs}Register container node functors node id {node.id} {node.description} : {len(functors)} functors"
            )
    ...
    def process_functor(
        self, functor, event, value, manual=False, extra_data=None
    ) -> bool:
    ...
    def execute_node(self, node, event, value, manual=False, extra_data=None) -> bool:
        ...
        if verbose_id:
            syslog.info(
                f"{logTabs}EXEC:[{node.id}] [{node.nodeType.name}] {node.description}"
            )
        try:
            match node.nodeType:
                case ExecutionGraphNodeType.Group:
                    for child in node.children:
                        result = self.execute_node(
                            child, event, value, manual, extra_data
                        )
                    ...
                    for functor in functor_list:
                        ...
                        if verbose_id:
                            if isinstance(functor, gremlin.actions.ActivationCondition):
                                ...
                                syslog.info(
                                    f"{logTabs}>Executed activation condition {condition_name} result: {'PASS' if result else 'FAIL'}"
                                )
                            elif isinstance(functor, gremlin.actions.AbstractCondition):
                                condition_name = functor.condition_name()
                                syslog.info(
                                    f"{logTabs}>Executed condition {condition_name} result: {'PASS' if result else 'FAIL'}"
                                )
                            else:
                                syslog.info(
                                    f"{logTabs}> !!! Executed action {functor.__class__.__name__} result: {'PASS' if result else 'FAIL'}"
                                )
                        if not result:
                            break
    ...
    def execute_functor_id(
        self, id, event, value, manual=False, extra_data=None
    ) -> bool:
        """executes a functor chain
...
class VirtualButtonCallback:
    ...
    def __call__(self, event, value=None):
        ...
        if verbose:
            syslog.info(
                f"Send virtual button {event.is_pressed} ---------------------------------------------------------------------------"
            )
    ...
    def __init__(self, data):
        ...
        if isinstance(data, gremlin.base_buttons.VirtualAxisButton):
            self.virtual_button = gremlin.actions.AxisButton(
                data.lower_limit, data.upper_limit, data.direction
            )
...
class AbstractExecutionGraph(QtCore.QObject):
    ...
    graph_completed = QtCore.Signal(
        object
    )  # fires when the process events have been all processed - parameter - the grap object just completed
...
class ContainerExecutionGraph(AbstractExecutionGraph):
    ...
    def _build_graph(self, container, parent=None):
        ...
        if container.has_conditions:
            functor = self._create_activation_condition(
                container.activation_condition, container, is_container_condition=True
            )
...
class ActionSetExecutionGraph(AbstractExecutionGraph):
    ...
    def _build_graph(self, action_set, parent=None):
        ...
        for action in action_set:
            ...
            if verbose:
                syslog.info(
                    f"\tadding action: {type(action)} priority: {priority} data: {str(action)}"
                )
        ...
        if verbose:
            ...
            for index, action in enumerate(ordered_action_set):
                ...
                syslog.info(
                    f"\t{index}: input type: {input_item.input_type} {input_stub} action: {type(action)}  data: {str(action)} "
                )
        ...
        for action in ordered_action_set:
            ...
            if action.has_conditions:
                functor = self._create_activation_condition(
                    action.activation_condition, action
                )
                self.functors.append(functor)
            ...
            if add_default_activation and not has_input_action:
                ...
                activation_condition = gremlin.base_conditions.ActivationCondition(
                    [condition], gremlin.actions.ActivationRule.All
                )
                functor = self._create_activation_condition(
                    activation_condition, action
                )
                self.functors.append(functor)
                sequence.append("Condition")
                nodes[action].functors.append(functor)
            ...
            functor: gremlin.base_conditions.AbstractFunctor = action.functor(
                action, nodes[action]
            )
            extra_inputs = functor.latch_extra_inputs()
            if extra_inputs:
