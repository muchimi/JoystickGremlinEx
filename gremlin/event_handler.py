...
class Event:
    ...
    def __init__(
        ...
        if __debug__ and event_type in (InputType.Keyboard, InputType.KeyboardLatched):
            if virtual_code == 0:
                if isinstance(identifier, gremlin.keyboard.Key):
    ...
    def getInputType(self):
        ...
        if self.extra_data and "override_input_type" in self.extra_data:
            return self.extra_data["override_input_type"]
    ...
    def __str__(self):
        ...
        if self.event_type == InputType.Mouse:
            ...
            scan_code, is_extended = self.identifier
            key = gremlin.keyboard.Key(scan_code=scan_code, is_extended=is_extended)
            stub = f"Keyboard - scan code, extended :  [{key.name}]  vk: {key.virtual_code} (0x{key.virtual_code:X}) pressed: {self.is_pressed}"
        ...
        return (
            f"Event: [{self._id}] device: [{device_stub}] event type: [{self.event_type.name} ({self.event_type})] - {stub}  callback key: [{self.callbackKey}]"
        )
...
class EventListener(QtCore.QObject):
    ...
    profile_before_start = Signal()  # occurs before the profile start signal
    ...
    set_virtual_button_usage = Signal(
        dinput.DeviceSummary, int, bool, object
    )  # called when a button state should be set (device_guid, virtual_id : int, state : bool, key) - the key is the action key
    ...
    def _dinput_event_handler(self, data):
        ...
        if is_virtual:
            ...
            if gremlin.shared_state.current_profile.settings.getVjoyAsInput(vjoy_id):
    ...
    def _mouse_handler(self, event):
        ...
        if not event.is_injected:
            ...
            # record the mouse state as a key
            key_id = (event.button_id + 0x1000, False)
...
class LatchedCallbackData:
    """holds callback data for latched data"""

    def __init__(self, input_item, event_type, callback, action_data=None):
        self.event_type = event_type
...
class EventHandler(QtCore.QObject):
    ...
    def reset(self):
        ...
        self.latched_input_map = {}  # map of latched events to the input item
    ...
    def _dump_latched_input_map(self):
        """dumps the latched input map for debugging purposes"""

        syslog.info("Latched Input Map Dump - START ----------------------")
        for device_guid, modes in self.latched_input_map.items():
            for mode, keys in modes.items():
                data: LatchedCallbackData
                for key, data in keys.items():
                    device = gremlin.joystick_handling.getDevice(device_guid)
                    key_stub = self._callback_key_stub(key)
                    syslog.info(
                        f"\tDevice: [{device.name}] device id:[{device.device_id}] Mode: [{mode}] Input Item: [{data.input_item.display_name}]  has callback: [{data.callback is not None}] Event input type: [{data.event_type.name}] Key: {key_stub}"
                    )
        syslog.info("Latched Input Map Dump - END DUMP ----------------------")

    def _callback_key_stub(self, key):
        key_device_guid, key_event_type, key_data, latched = key
        key_device = gremlin.joystick_handling.getDevice(key_device_guid)
        if key_event_type in (InputType.KeyboardLatched, InputType.Keyboard):
            scan_code, is_extended = key_data
            k = gremlin.keyboard.Key(scan_code=scan_code, is_extended=is_extended)
            stub = k.debug_name
        else:
            stub = ""
    ...
    def addCallback(self, device_guid: dinput.GUID, mode: str, event: Event, callback: Callable, permanent=False, node=None, extra_data: dict = {}):
        ...
        if latched:
            ...
            if device_guid not in self.latched_input_map:
                self.latched_input_map[device_guid] = {}
            ...
            if verbose:
                device = gremlin.joystick_handling.getDevice(device_guid)
                syslog.info(f"CALLBACK: latched callback - REGISTERED LATCH - key[{key}] input item: [{input_item.display_name}]")
            # do not register latched callbacks as regular callbacks
            return
        ...
        if event:
            if event.event_type in (InputType.Keyboard, InputType.KeyboardLatched, InputType.Mouse):
                ...
                for key in key_list:
                    ...
                    if device_guid not in self.latched_keyboard_key_map:
                        self.latched_keyboard_key_map[device_guid] = {}
                    if mode not in self.latched_keyboard_key_map[device_guid]:
                        self.latched_keyboard_key_map[device_guid][mode] = {}
                    if keyid not in self.latched_keyboard_key_map[device_guid][mode]:
                        self.latched_keyboard_key_map[device_guid][mode][keyid] = []
                    data = self.latched_keyboard_key_map[device_guid][mode][keyid]
                    data.append((self._install_plugins(callback), permanent))
                    ...
                    if input_item not in self.latched_events[device_guid][mode][keyid]:
                        self.latched_events[device_guid][mode][keyid][input_item] = []

                    self.latched_events[device_guid][mode][keyid][input_item].append(identifier)
                    if verbose:
                        syslog.info(
                            f"Key latch registered by guid [{device_guid}]  mode: [{mode}] input item: [{input_item.display_name}] vk: [{virtual_code} (0x{virtual_code:X})] source keyid: [{gremlin.keyboard.KeyMap.keyid_tostring(keyid_source)}]-> translated keyId: [{gremlin.keyboard.KeyMap.keyid_tostring(keyid)}] name: {key.name} Primary key: [{primary_key}]"
                        )
                    pass
                ...
                if latched:
                    ...
                    for perm in perms:
                        ...
                        if input_item not in self.latched_keys[perm]:
                            self.latched_keys[perm][input_item] = []
                        if callback not in self.latched_keys[perm][input_item]:
                            self.latched_keys[perm][input_item].append((primary_key, callback))
    ...
    def execute_event(self, event: Event, skip_execute=False):
        ...
        try:
            ...
            if isinstance(data, LatchedCallbackData):
                ...
                if action_data:
                    if event.extra_data is None:
                        event.extra_data = {}
                    event.extra_data["action_data"] = action_data
            ...
            if verbose:
                syslog.info(
                    f"EVENT: Matching input items: {input_item.display_name if input_item else 'None'}  callback found: [{'Yes' if callback else 'No'}]  event: {str(event)}"
                )
            ...
            if event.event_type in (
                ...
                elif isinstance(event_key, gremlin.types.MouseButton):
                    key = gremlin.keyboard.Key()
                    key.mouse_button = event_key
                    event_key = key

                latched_map = {}

                # see if matching the input item
                for input_id in latched_inputs:
                    if not isinstance(input_id, gremlin.keyboard.Key):
                        continue

                    latch_found = False
                    latched_keys = input_id.getKeys()
                    m_list = []
                    f_list = []

                    if event.extra_data is None:
                        event.extra_data = {}
                    event.extra_data["input_item"] = input_item
                    event.extra_data["latched_key"] = input_id  # key sequence that triggered the callback
                    event.extra_data["override_input_type"] = InputType.JoystickButton
                    ...
                    if latched_keys:
                        ...
                        if verbose:
                            ...
                            for index, key in enumerate(latched_keys):
                                syslog.info(f"\t[{index}]: {key.name} pressed: {keyPressed(key)}")

                        # process release events if any
                        if not event.is_pressed:
                            for key in latched_keys:
                                if key in self._keyboard_callback_map:
                                    r_list = self._keyboard_callback_map[key]
                                    del self._keyboard_callback_map[key]
                                    self._trigger_callbacks(r_list, event)
                    ...
                    if latch_found and latched_map:
                ...
                m_list = self._matching_callbacks(event, input_item)
                f_list = self._matching_functors(event)
                if verbose_detailed and not (m_list or f_list):
    ...
    def _matching_streamdeck_callbacks(self, event):
        ...
        if event.event_type == InputType.StreamDeck:
            ...
            if verbose and not callback_list:
                syslog.info(f"EVENT: STREAMDECK: no callbacks for key: [{key}] mode: [{self.runtime_mode}]. Normal if this button has no mappings.")
    ...
    def _mode_lookup_chain(self, event: Event):
        ...
        if event.extra_data:
            add_mode(event.extra_data.get("mode"))
            add_mode(event.extra_data.get("target_mode"))
    ...
    def _dump_matching_callbacks(self, event: Event, input_item):
        """Dumps the matching callbacks for the given event and input item."""
        callback_list = self._matching_callbacks(event, input_item)
        syslog.info(f"Dumping matching callbacks for event: {event} and input item: {input_item}")
        for callback in callback_list:
            syslog.info(f"\tCallback: {callback}")
        syslog.info("Finished dumping matching callbacks.")

    def _matching_callbacks(self, event: Event, input_item):
        ...
        if device_guid in self.latched_callbacks:
            for mode in mode_list:
                device_modes = self.latched_callbacks.get(device_guid, {})
                if mode in device_modes and key in device_modes[mode]:
                    if input_item in device_modes[mode][key]:
                        callback_list = device_modes[mode][key][input_item]
                        callback_found = True
                    if verbose:
                        event = self.callback_key_map[key]
                        self.dump_exectree(device_guid, mode, event)

        if not callback_found and device_guid in self.callbacks:
            for mode in mode_list:
                device_modes = self.callbacks.get(device_guid, {})
                if mode in device_modes and key in device_modes[mode]:
                    callback_list = device_modes[mode][key]
                    if verbose:
                        event = self.callback_key_map[key]
                        self.dump_exectree(device_guid, mode, event)
                    break
...
class JoystickState:
    ...
    def reset(self):
        ...
        for dev in gremlin.joystick_handling.all_joystick_devices():
            ...
            if dev.product_id == 59094 and dev.vendor_id == 1240 and dev.name == "IFR1":
                ...
                is_input_enabled = current_profile.settings.getVjoyAsInput(vid=dev.vjoy_id) if current_profile else False
    ...
    def getVjoyAsInput(self, vid: int) -> bool:
        """true if vjoy device is also used as input"""
        profile = gremlin.shared_state.current_profile
        if profile:
            return profile.settings.getVjoyAsInput(vid)
        return False

    def setVjoyAsInput(self, vid: int, enabled: bool):
        profile = gremlin.shared_state.current_profile
        if profile:
            profile.settings.setVjoyAsInput(vid, enabled)
...
class JoystickEventProcessor:
    ...
    def __init__(self):
        ...
        self._group_map = {}  # map of group to callbacks [group_key] -> [list of callbacks]
    ...
    def registerListenerUICallback(
        ...
        for mode in mode_list:
            ...
            if group:
                ...
                if callback not in self._group_map[group]:
                    self._group_map[group].append((callback, source, mode, device_guid, input_type, input_id))
    ...
    def unregisterGroup(self, group):
        """removes all callbacks associated with a group"""
        if group in self._group_map:
            for callback, source, mode, device_guid, input_type, input_id in self._group_map[group]:
                self.unregisterListenerUICallback(
                    device_guid=device_guid,
                        ...
                        for l_input_type in self._listener_callbacks[source][l_mode][l_device_guid]:
                            for l_input_id in self._listener_callbacks[source][l_mode][l_device_guid][l_input_type]:
                                if callback in self._listener_callbacks[source][l_mode][l_device_guid][l_input_type][l_input_id]:
                                    ...
                                    if verbose:
                                        syslog.info(
                                            f"JEP: remove listener: source: [{source.name}] callback: [{callback.__module__}.{callback.__self__.__class__.__name__}.{callback.__name__}]"
                                        )
