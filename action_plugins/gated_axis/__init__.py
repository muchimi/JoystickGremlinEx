...
class QGatedAxisWidget(QtWidgets.QWidget):
    ...
    def __init__(self, action_data, show_configuration=False, show_output_mode=False, object_name=None, parent=None):
        ...
        display_event_widget = gremlin.ui.ui_common.QDataCheckbox(
            value=self._option_display_events, callback=self._handle_display_event_changed, tooltip="Hide or show events", label="Show events"
        )
...
making changes that impact the order of gates or ranges."""
    ...
    def _show_help(self):
        ...
        if location is not None and os.path.isfile(location):
            ...
            h = 400
            geom = self.geometry()
            dialog.setGeometry(int(geom.x() + geom.width() / 2 - w / 2), int(geom.y() + geom.height() / 2 - h / 2), w, h)
    ...
    def _reload_ranges(self):
        ...
        for index, rng in enumerate(range_list):
            widget = RangeInfoWidget(index + 1, rng, decimals, self._configure_range_cb_ui, parent=self.range_flow_widget)
    ...
    def _configure_range_exec(self, rng: RangeInfo):
        ...
        if connected:
            ...
            dialog = gremlin.gated_handler.GateConditionEditorDialog(
                gate_data=self._gate_data, info_object=rng, action_data=self.action_data, input_type=InputType.JoystickAxis
            )
    ...
    def _slider_range_context_cb_ui(self, value):
        ...
        widget = gremlin.ui.ui_common.getVContainer([QtWidgets.QLabel(msg), line], widget_only=True)
    ...
    def _slider_gate_configure_cb(self, handle_index):
        ...
        dialog = gremlin.gated_handler.GateConditionEditorDialog(
            gate_data=gate.parent, info_object=gate, action_data=self.action_data, input_type=InputType.JoystickButton
        )
    ...
    def _create_filter_widgets(self):
        ...
        for _, trigger in enumerate(TriggerMode):
            ...
            widget = gremlin.ui.ui_common.QDataCheckbox(
                label=TriggerMode.to_display_name(trigger), data=trigger, callbackEx=self._filter_cb, value=self._gate_data.filter_map[trigger]
            )
...
class GatedAxisWidget(gremlin.input_item.AbstractActionWidget):
    ...
    def _create_ui(self):
        ...
        self.gate_widget = QGatedAxisWidget(action_data=self.action_data, show_configuration=False, parent=self, object_name=object_name)
