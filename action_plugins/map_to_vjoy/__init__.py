...
class VJoyRemapWidget(gremlin.input_item.AbstractActionWidget):
    ...
    def _create_ui(self):
        ...
        try:
            ...
            el.vjoy_button_usage.connect(self._vjoy_button_usage_changed) # listen to grid button changes # called when a button actually flips
    ...
    def _create_range_widgets(self):
        ...
        for (label, data, tooltip) in quick_list:
            widget = gremlin.ui.ui_common.QDataPushButton(label, data=data, tooltip=tooltip, callback = self._handle_button_range_quick_set)
            widgets.append(widget)
    ...
    def _create_input_axis(self):
        ...
        self.container_reverse_widget = gremlin.ui.ui_common.getHContainer(self.reverse_checkbox, min_height = self.container_height, widget_only = True, left_margin = 12)
        ...
        self.container_output_mode_widget = gremlin.ui.ui_common.getHContainer(widgets, "Output mode:", min_height = self.container_height, widget_only = True, left_margin = 12)
        ...
        self.container_relative_widget = gremlin.ui.ui_common.getHContainer(widgets, "Relative mode options:", min_height = self.container_height, widget_only = True, left_margin = 12)
    ...
    def _create_output_range(self):
        ...
        self.container_output_curve_widget = gremlin.ui.ui_common.getHContainer(widgets,"Output Curve Options", min_height = self.container_height, widget_only = True, left_margin = 12)
    ...
    def _create_selector(self):
        ...
        self.container_mode_selector_widget = gremlin.ui.ui_common.getGridContainer(["Mode:", self.cb_action_list, self.action_label], widget_only=True, bottom_margin = 4)
        ...
        self.container_device_selector_widget =  gremlin.ui.ui_common.getGridContainer([self.lbl_vjoy_device_selector, self.cb_vjoy_device_selector, " "], widget_only=True, bottom_margin = 4)
        ...
        self._next_unused_widget = gremlin.ui.ui_common.QDataPushButton("Next Unused",
        ...
        self.container_output_selector_widget =  gremlin.ui.ui_common.getGridContainer([self.lbl_vjoy_output_selector, self.cb_vjoy_output_selector, self._next_unused_widget], widget_only=True, bottom_margin = 4)
        grids.append(self.container_output_selector_widget)

        self.container_hat_selector_widget =  gremlin.ui.ui_common.getGridContainer([self.lbl_hat_selector, self.cb_hat_selector, " "], widget_only=True, bottom_margin = 4)
        grids.append(self.container_hat_selector_widget)

        self.container_hat_return_selector_widget =  gremlin.ui.ui_common.getGridContainer([self.lbl_hat_return_selector, self.cb_hat_return_selector, " "], widget_only=True, bottom_margin = 4)
        ...
        self.cb_vjoy_output_selector.currentIndexChanged.connect(self._vjoy_input_id_changed)
    ...
    def _vjoy_input_id_changed(self, index):
        ...
        if self.cb_vjoy_output_selector.count():
            ...
            with QtCore.QSignalBlocker(self.cb_vjoy_output_selector):
                if self.action_data.action_mode in (VjoyAction.VJoyHat, VjoyAction.VJoyHatPress, VjoyAction.VJoyHatPulse) and self.action_data.input_type == InputType.JoystickButton:
                    # hat output
    ...
    def _handle_tag_callback(self, action : VjoyRemap, extra_data : dict):
        if action.vjoy_id == self.action_data.vjoy_id:
            # same vjoy device
            input_type = action._get_input_type() if hasattr(action,"_get_input_type") else action.input_type if hasattr(action,"input_type") else None # ignore
            ...
            if action_mode in (VjoyAction.VJoySetAxis, VjoyAction.VJoySetAxisStepped, VjoyAction.VJoyRangeAxis, VjoyAction.VJoyAxis, VjoyAction.VJoyInvertAxis, VjoyAction.VJoyMergeAxis):
                ...
                if action_mode in (VjoyAction.VJoyButton, VjoyAction.VJoyButtonPress, VjoyAction.VJoyButtonRelease, VjoyAction.VJoyPulse, VjoyAction.VJoyToggle, VjoyAction.VJoyAxisToButton, VjoyAction.VJoyHatToButton):
                    ...
                    if input_id < 1 or input_id > count:
                        ...
                        return
                elif action_mode in (VjoyAction.VJoyHat, VjoyAction.VJoyHatPress, VjoyAction.VJoyHatPulse):
                    # map to hat
                    count = dev.hat_count
    ...
    def _update_ui(self):
        ...
        if is_axis:
            ...
            hat_position_visible = action in (VjoyAction.VJoyHat, VjoyAction.VJoyHatPress, VjoyAction.VJoyHatPulse)
            hat_return_position_visible = action in (VjoyAction.VJoyHat, VjoyAction.VJoyHatPulse)
    ...
    def select_button(self, vjoy_id, button_id, emit = False):
        ...
        if self._last_button_id != -1 and self._last_button_id != button_id:
            ...
            if verbose: syslog.info(f"VJOYREMAP: send button clear {vjoy_id} {self._last_button_id} {self.action_data.id}")
        ...
        if verbose: syslog.info(f"VJOYREMAP: send button select {vjoy_id} {button_id} {self.action_data.id}")
...
class VJoyRemapFunctor(gremlin.base_profile.AbstractFunctor):
        ...
        if self.input_type == InputType.JoystickAxis:
                ...
                case VjoyAction.VJoyAxis:
                    ...
                    match self.action_data.sync_mode:
                        case SyncMode.Default:
                            trigger = True
                            if verbose: syslog.info(f"{vjoy_stub} default : {self.action_data.state.default_value}")
                            value = self.action_data.axis_start_value
                        case SyncMode.Input:
                            trigger = True
                            if verbose: syslog.info(f"VJOY REMAP SYNC: {device_stub} {vjoy_stub} input : {value:0.3f}")
                            value = value
                        case SyncMode.LastOrInput:
                            trigger = True
                            last = self.action_data.axis_last_value
                            if last is None:
                                if verbose: syslog.info(f"VJOY REMAP SYNC: {vjoy_stub} last or input : use input value : {value:0.3f}")
                            else:
            ...
            match self.action_data.sync_mode:
                case SyncMode.Default:
                    ...
                    if verbose: syslog.info(f"{vjoy_stub} default : {value}")
                ...
                case SyncMode.LastOrInput:
                    ...
                    if last is None:
                        if verbose: syslog.info(f"{vjoy_stub} last or input : use input value : {is_pressed}")
                        value = is_pressed
                    else:
                        if verbose: syslog.info(f"{vjoy_stub} last or input : use last value : {last:0.3f}")
                        value = last
                case SyncMode.LastOrDefault:
                    last = self.action_data.button_last_value
                    if last is None:
                        value = self.action_data.button_start_value
                        if verbose: syslog.info(f"{vjoy_stub} last or input : use default value : {value}")

                    else:
                        if verbose: syslog.info(f"{vjoy_stub} last or default: use last value : {last}")
                        value = last
                case SyncMode.Ignore:
            ...
            if trigger:
                ...
                if self.action_data.action_mode in (VjoyAction.VJoyHat, VjoyAction.VJoyHatPress, VjoyAction.VJoyHatPulse):
                    # hat output
                    ...
                    if is_remote:
                        self.remote_client.send_hat(self.vjoy_id, self.vjoy_input_id, position, client_list = self.client_list, force_remote = is_paired )
                    ...
                    if is_remote or is_paired:
                        self.remote_client.send_button(self.vjoy_id, self.vjoy_input_id, value, client_list = self.client_list, force_remote = is_paired)
    ...
    def profile_stop(self):
        ...
        if self.input_type in VJoyRemapWidget.input_type_buttons:
            ...
            is_paired = gremlin.remote.remote_control.paired
            action_mode = self.action_data.action_mode
            if action_mode in (VjoyAction.VJoyHat, VjoyAction.VJoyHatPress, VjoyAction.VJoyHatPulse):
                # hat modes
                ...
                if is_remote:
                    self.remote_client.send_hat(self.vjoy_id, self.vjoy_input_id, position, client_list = self.client_list, force_remote = is_paired )
            elif action_mode in (VjoyAction.VJoyButton, VjoyAction.VJoyButton.VJoyButtonPress, VjoyAction.VJoyButton.VJoyButtonRelease, VjoyAction.VJoyButtonInverted):
                # button modes
                is_pressed = False
                if is_local:
                    if gremlin.joystick_handling.is_vjoy_connected(self.vjoy_id):
                ...
                if is_remote or is_paired:
                    self.remote_client.send_button(self.vjoy_id, self.vjoy_input_id, is_pressed, client_list = self.client_list, force_remote = is_paired)
        ...
        if self.action_data.action_mode == VjoyAction.VJoySetAxisStepped:
            ...
            match input_type:
                case InputType.JoystickButton:
                    ...
                    if is_remote:
                        self.remote_client.send_button(device_id, input_id, True, client_list = self.client_list, force_remote = force_remote)
                case InputType.JoystickHat:
                    if self.verbose_extra: syslog.info(f"pulse ON vjoy [{device_id}] hat [{input_id}] position: [{position}]")
                    if is_local:
                        if gremlin.joystick_handling.is_vjoy_connected(self.vjoy_id):
                            joystick_handling.VJoyProxy()[self.vjoy_id].hat(self.vjoy_input_id).direction = position
                    if is_remote:
                        self.remote_client.send_hat(self.vjoy_id, self.vjoy_input_id, position, client_list = self.client_list, force_remote = force_remote )
    ...
    def _pulse_off(self, data):
        ...
        match input_type:
            case InputType.JoystickButton:
                ...
                if is_remote:
                    self.remote_client.send_button(device_id, input_id, False, client_list = self.client_list, force_remote = force_remote)

            case InputType.JoystickHat:
                    position = self.action_data.vjoy_hat_return_position

                    if self.verbose_extra: syslog.info(f"pulse OFF vjoy [{device_id}] hat [{input_id}] position: [{position}]")
                    if is_local:
                        if gremlin.joystick_handling.is_vjoy_connected(self.vjoy_id):
                            joystick_handling.VJoyProxy()[self.vjoy_id].hat(self.vjoy_input_id).direction = position
                    if is_remote:
    ...
    def pulse_start(self, device_id : int, input_type: InputType, input_id : int, position : tuple = None, duration : float = 0, interval : float = 0, is_local : bool = True, is_remote : bool = False, force_remote : bool = False):
        ''' pulse setup '''
        verbose = self.verbose
        if verbose: syslog.info(f"Pulse START vjoy {device_id} input type: {input_type.name} input id {input_id} position: {position} duration: {duration:0.3f} interval: {interval:0.3f}")
        ...
        if key in self.pulse_worker_map:
            ...
            if worker.is_running:
                ...
                if verbose: syslog.info(f"\talready pulsing - ignored")
                return
        else:
            args = (device_id, input_type, input_id, position, is_local, is_remote, force_remote)
        ...
        if verbose: syslog.info(f"\activate")
        worker.start()

    def pulse_stop(self, device_id : int, input_type: InputType, input_id : int, position : tuple = None):
    ...
    def _relative_pulse_on(self, data):
            ...
            with self.lock:
                ...
                if is_remote:
                    self.remote_client.send_relative_axis(self.vjoy_id, self.vjoy_input_id, value, client_list = self.client_list)
        ...
        if event.is_axis: # self.input_type == InputType.JoystickAxis:
            ...
            match self.action_mode:
                case VjoyAction.VJoyAxisToButton:
                        ...
                        case ButtonOutputMode.Pulse:
                            ...
                            if in_range:
                                if not self.in_range:
                                    ...
                                    self.pulse_start(device_id, input_type, input_id, position, self.pulse_delay/1000, repeat_interval, is_local, is_remote, force_remote)
                                    auto_complete = False
                            else:
                                if self.in_range:
                                    self.in_range = False
                    ...
                    if is_pressed is not None:
                        if verbose: syslog.info(f"VJOY: set device [{self.vjoy_id}] button {self.vjoy_input_id} pressed: {is_pressed}")
                        self.action_data.button_last_value = is_pressed
                        if is_local:
                            if gremlin.joystick_handling.is_vjoy_connected(self.vjoy_id):
                                joystick_handling.VJoyProxy()[self.vjoy_id].button(self.vjoy_input_id).is_pressed = is_pressed
                        if is_remote:
                            self.remote_client.send_button(self.vjoy_id, self.vjoy_input_id, is_pressed, client_list = self.client_list)
            ...
            if input_id > 0:
                        ...
                        if is_pressed:
                            ...
                            for pressed_position in pressed_positions:
                                ...
                                if release_input_id > 0:
                                    ...
                                    if is_remote:
                                        self.remote_client.send_button(device_id, release_input_id, False, client_list = self.client_list)
                ...
                for pressed_position in pressed_positions:
                    ...
                    if input_id > 0:
                        if verbose: syslog.info(f"VJOY: set device [{self.vjoy_id}] button {input_id} pressed: False")
                        if is_local:
                            if gremlin.joystick_handling.is_vjoy_connected(self.vjoy_id):
                                joystick_handling.VJoyProxy()[self.vjoy_id].button(input_id).is_pressed = False
                        if is_remote:
                            self.remote_client.send_button(self.vjoy_id, input_id, False, client_list = self.client_list)
            ...
            if self.action_mode in (VjoyAction.VJoyButton, VjoyAction.VJoyButtonInverted):
                ...
                if not is_pressed and self.action_data.exec_on_release:
                    ...
                    if is_remote or is_paired:
                        self.remote_client.send_button(self.vjoy_id, self.vjoy_input_id, pressed_value, client_list = self.client_list, force_remote = force_remote)
                ...
                if trigger:
                    if is_local:
                        if gremlin.joystick_handling.is_vjoy_connected(self.vjoy_id):
                            ...
                            joystick_handling.VJoyProxy()[self.vjoy_id].button(self.vjoy_input_id).is_pressed = pressed_value
                    if is_remote or is_paired:
                        self.remote_client.send_button(self.vjoy_id, self.vjoy_input_id, pressed_value, client_list = self.client_list, force_remote = is_paired )
                else:
                ...
                if verbose: syslog.info(f"VJOY: hold hat device [{self.vjoy_id}] hat {self.vjoy_input_id} position: {self.action_data.vjoy_hat_position}")
                if is_pressed:
                ...
                if verbose: syslog.info(f"VJOY: set hat device [{self.vjoy_id}] hat {self.vjoy_input_id} position: {self.action_data.vjoy_hat_position}")
                direction = self.action_data.vjoy_hat_position
                if is_local:
                    if gremlin.joystick_handling.is_vjoy_connected(self.vjoy_id):
                ...
                # hat ouput when mapped to a button (press only)
                if verbose: syslog.info(f"VJOY: pulse hat device [{self.vjoy_id}] hat {self.vjoy_input_id} position: {self.action_data.vjoy_hat_position}")
                ...
                if fire_event:
                    ...
                    if is_remote or is_paired:
                        self.remote_client.send_button(self.vjoy_id, self.vjoy_input_id, True, client_list = self.client_list, force_remote = is_paired )


            elif self.action_mode == VjoyAction.VJoyButtonRelease:
                # release button (no auto release)
                if verbose: syslog.info(f"VJOY: set device [{self.vjoy_id}] button {self.vjoy_input_id} pressed: False")
                if fire_event:
                    ...
                    if is_remote or is_paired:
                        self.remote_client.send_button(self.vjoy_id, self.vjoy_input_id, False, client_list = self.client_list, force_remote = is_paired )
                    ...
                    if input_type in [InputType.JoystickButton, InputType.Keyboard] \
                        ...
                        if is_local:
                            if gremlin.joystick_handling.is_vjoy_connected(self.vjoy_id):
                                ...
                                if verbose: syslog.info(f"VJOY: set device [{self.vjoy_id}] button {input_id} pressed: {button.is_pressed}")
                        if is_remote:
                    ...
                    if primary or latched:
                        ...
                        if trigger:
                            ...
                            if self.action_data._target_step_linear_mode:
                                ...
                                self.pulse_start(self.vjoy_id, input_type, self.vjoy_input_id, position, 0, self.repeat_interval, is_local, is_remote, force_remote)
                            else:
                ...
                if fire_event:
                    self.action_data.button_last_value = is_pressed
                    if is_local:
                        if verbose: syslog.info(f"VJOY: send local button [{self.vjoy_id}] button: {self.vjoy_input_id} pressed: {is_pressed}")
                        joystick_handling.VJoyProxy()[self.vjoy_id].button(self.vjoy_input_id).is_pressed = is_pressed
                    if is_remote:
                        if verbose: syslog.info(f"VJOY: send remote button [{self.vjoy_id}] button: {self.vjoy_input_id} pressed: {is_pressed}")
                        self.remote_client.send_button(self.vjoy_id, self.vjoy_input_id, is_pressed, client_list = self.client_list)
            ...
            if verbose: syslog.info(f"VJOY: set device [{self.vjoy_id}] hat {self.vjoy_input_id} direction: {action_value.current}")
            if is_local:
                if self.verbose: syslog.info(f"VJOY: send local hat: vjoy [{self.vjoy_id}] hat [{self.vjoy_input_id}] value: {action_value.current}")
                joystick_handling.VJoyProxy()[self.vjoy_id].hat(self.vjoy_input_id).direction = action_value.current
            if is_remote:
                if self.verbose: syslog.info(f"VJOY: send remote hat: vjoy [{self.vjoy_id}] hat [{self.vjoy_input_id}] value: {action_value.current}")
                self.remote_client.send_hat(self.vjoy_id, self.vjoy_input_id, action_value.current, client_list = self.client_list)
    ...
    def _set_axis(self, vid : int, axis_id : int , value : float):
        ...
        if is_local:
            ...
            joystick_handling.VJoyProxy()[self.vjoy_id].axis(self.vjoy_input_id).value = value
        if is_remote:
            if self.verbose: syslog.info(f"VJOY: send remote axis: vjoy [{self.vjoy_id}] axis [{self.vjoy_input_id}] value: {value:0.03f}")
        ...
        if gremlin.joystick_handling.is_vjoy_connected(self.vjoy_id):
            ...
            while self.thread_running:
                try:
                    ...
                    if is_local:
                        if self.verbose: syslog.info(f"VJOY: send local relative axis: vjoy [{self.vjoy_id}] axis [{self.vjoy_input_id}] value: {value:0.03f}")
                        vjoy_dev.axis(self.vjoy_input_id).value = value
                    if is_remote:
                        if self.verbose: syslog.info(f"VJOY: send remote relative axis: vjoy [{self.vjoy_id}] axis [{self.vjoy_input_id}] value: {value:0.03f}")
                        self.remote_client.send_axis(self.vjoy_id, self.vjoy_input_id, value, client_list = self.client_list)
                    # remember the last value
                    self.action_data.axis_last_value = value
...
'''
    ...
    def __init__(self, parent):
        ...
        self.vjoy_hat_return_position = (0,0) # hat return position - center is the default
    ...
    def _get_input_type(self):
        ...
        if self.action_mode in (VjoyAction.VJoySetAxis, VjoyAction.VJoyInvertAxis, VjoyAction.VJoyAxis, VjoyAction.VJoyMergeAxis, VjoyAction.VJoySetAxisStepped):
            input_type = InputType.JoystickAxis
        elif self.action_mode in (VjoyAction.VJoyHat, VjoyAction.VJoyHatPress, VjoyAction.VJoyHatPulse):
    ...
    def icon(self):
        ...
        match input_type:
            ...
            case InputType.JoystickButton:
                if self.action_mode in (VjoyAction.VJoyHat, VjoyAction.VJoyHatPress, VjoyAction.VJoyHatPulse):
                    position = (0,0)
