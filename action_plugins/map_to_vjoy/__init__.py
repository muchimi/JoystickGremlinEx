...
import gremlin.util
from gremlin.repeater import AcceleratedEncoder, PulseWorker
...
from gremlin.types import ButtonOutputMode, SendType, SyncMode, VjoyAction, DeviceType
import vjoy.vjoy
...
class EncoderMode(enum.Enum):
    Relative = 0  # relative mode - increments based on encoder input
    Spring = 1  # spring mode - returns the axis to the target position after the encoder stops moving

    @staticmethod
    def from_string(mode_str: str) -> EncoderMode:
        try:
            match mode_str.casefold():
                case "relative":
                    return EncoderMode.Relative
                case "spring":
                    return EncoderMode.Spring
            return EncoderMode[mode_str]
        except KeyError:
            return EncoderMode.Relative

    @staticmethod
    def to_string(mode: EncoderMode) -> str:
        return mode.name.casefold()

    @staticmethod
    def tooltip(mode: EncoderMode) -> str:
        match mode:
            case EncoderMode.Relative:
                return "Relative mode - increments based on encoder input"
            case EncoderMode.Spring:
                return "Spring mode - returns the axis to the target position after the encoder stops moving"
        return ""


class StepMode(enum.Enum):
    NotSet = 0
    Velocity = 1  # while input is pressed, accelerates at the specified velocity
    Stepped = 2  # stepped mode - increments in discrete steps
    Encoder = 3  # encoder mode - increments based on encoder input - this counts encoder steps and keeps on going until input stops - uses a delay to determine when to stop after the last encoder trigger

    @staticmethod
    def from_string(mode_str: str) -> StepMode:
        try:
            match mode_str.casefold():
                case "velocity":
                    return StepMode.Velocity
                case "stepped":
                    return StepMode.Stepped
                case "encoder":
                    return StepMode.Encoder
            return StepMode[mode_str]
        except KeyError:
            return StepMode.NotSet

    @staticmethod
    def to_string(mode: StepMode) -> str:
        return mode.name.casefold()

    @staticmethod
    def tooltip(mode: StepMode) -> str:
        match mode:
            case StepMode.NotSet:
                return "Step mode not set"
            case StepMode.Velocity:
                return "While input is pressed, accelerates at the specified velocity"
            case StepMode.Stepped:
                return "Stepped mode - increments in discrete steps"
            case StepMode.Encoder:
                return "Encoder mode - increments based on encoder input"
        return ""
...
class MergeWidget(gremlin.ui.ui_common.QDataWidget):
    """single merge step widget  - lets the user pick a device and axis"""
    ...
    def __init__(self, data, label: str = None, filter_input=None, action_data=None, parent=None):
        ...
        self.id = gremlin.util.get_guid()  # unique ID of this widget, also the group ID of this widget for callback tracking
        ...
        el.vjoy_as_input_changed.connect(self._handle_vjoy_as_input_changed)
        ...
        self.merge_axis_repeater_label_widget = None
    ...
    def _handle_vjoy_as_input_changed(self, vjoy_id: int, enabled: bool):
        gremlin.util.InvokeUiMethod(self.updateDevices)
    ...
    def updateDevices(self):
        ...
        with QtCore.QSignalBlocker(self.merge_selector_device_widget):
            self.merge_selector_device_widget.clear()
            for dev in devices:
...
class GridPopupWindow(gremlin.ui.ui_common.QShowAtCursorDialog):
    def __init__(self, device_guid, input_type, vjoy_input_id, parent=None):
        ...
        usage_data = gremlin.joystick_handling.VirtualDeviceUsageState()
        action_map = usage_data.get_action_map(device_guid, input_type, vjoy_input_id)
        if not action_map:
            self.close()
        ...
        source_widget = gremlin.ui.ui_common.getHContainer(
            QtWidgets.QLabel(f"Vjoy {device_guid} Button {vjoy_input_id} mapped by:"),
            widget_only=True,
        )
...
class VJoyRemapWidget(gremlin.input_item.AbstractActionWidget):
    ...
    def _create_ui(self):
        ...
        self.key_map = {}
        self.keys = []
        self._stepped_mode_map = {}  # stepped mode page map
        ...
        try:
            ...
            self.usage_state = gremlin.joystick_handling.VirtualDeviceUsageState()
    ...
    def updateDeviceHooks(self):
        """hooks merged axis events to update the UI and value"""

        js = gremlin.event_handler.JoystickEventProcessor()
        for data in self.action_data._merge_data:
            # blitz current callbacks, if any
            js.unregisterGroup(data.id)
            js.registerListenerUICallback(
                device_guid=data.device_guid, input_type=InputType.JoystickAxis, input_id=data.input_id, callback=self._joystick_event_handler, group=data.id
            )
    ...
    def _joystick_event_handler(self, event: gremlin.event_handler.Event):
        ...
        if self.action_data.action_mode == VjoyAction.VJoyMergeAxis:
            ...
            # compute the merge value
            value = self.action_data.get_merged_axis_value(event)
    ...
    def _update_info(self):
        ...
        match self.action_data.action_mode:
            ...
            case VjoyAction.VJoySetAxisStepped | VjoyAction.VJoySetAxis:
                action_name = f"Vjoy device {vjoy_id} axis {vjoy_input_id}"
            ...
            case _:
                action_name = f"Vjoy [{self.action_data.action_mode.name}]"
        ...
        if is_axis:
            ...
            if hasattr(input_id, "display_name"):
                prefix = f"Input trigger {input_id.display_name}"
            else:
    ...
    def _create_selector(self):
        ...
        self.container_device_selector_widget = gremlin.ui.ui_common.getGridContainer(
            [self.virtual_device_label_widget, self.virtual_device_selector_widget, " "],
            widget_only=True,
            bottom_margin=4,
        )
        ...
        self.container_output_selector_widget = gremlin.ui.ui_common.getGridContainer(
            [
                self.virtual_output_label_widget,
                self.virtual_output_selector_widget,
    ...
    def _update_device_selector(self):
        """loads or reloads the output device list"""
        with QtCore.QSignalBlocker(self.virtual_device_selector_widget):
            self.virtual_device_selector_widget.clear()
            config = gremlin.config.Configuration()
            connected_only = not config.vjoy_show_disconnected
            device_list = gremlin.joystick_handling.virtual_devices(connected_only)
            for dev in device_list:
                self.virtual_device_selector_widget.addItem(dev.name, dev)

            virtual_device = self.action_data.virtual_device

            index = self.virtual_device_selector_widget.findData(virtual_device)
            if index != -1:
                self.virtual_device_selector_widget.setCurrentIndex(index)
            else:
                # change the action ID
                syslog.warning(
                    f"VJOY REMAP: output device [{virtual_device.name if virtual_device else 'None'}] not found in the available vjoy device list - resetting to [{self.virtual_device_selector_widget.currentData().name}]"
                )
                self.action_data.virtual_device = self.virtual_device_selector_widget.currentData()

            # update warning if needed
            self._handle_virtual_device_input_changed(self.virtual_device_selector_widget.currentIndex())
    ...
    def update_steps(self):
        gremlin.util.InvokeUiMethod(self._update_steps_ui)
    ...
    def _update_steps_ui(self):
        ...
        # latch configuration for stepping mode (hide if not enabled)
        self.latch_configuration_container.setVisible(self.action_data._stepped_latched)

        # select the visual for the current mode
        self.stepped_page_visualizer_container.setCurrentIndex(self._stepped_mode_map.get(self.action_data.step_mode, 0))
    ...
    def _create_step_ui(self):
        ...
        for mode in StepMode:
            if mode == StepMode.NotSet:
                continue
            widget = gremlin.ui.ui_common.QDataRadioButton(
                label=mode.name, value=self.action_data.step_mode == mode, callbackEx=self._handle_step_mode_changed, tooltip=StepMode.tooltip(mode), data=mode
            )
            widgets.append(widget)
    ...
    def _handle_step_mode_changed(self, widget, checked: bool):
        if checked:
            self.action_data.step_mode = widget.data
            self._update_ui()
    ...
    def _create_axis_stepping_ui(self):
        ...
        # velocity mode page
        container = self._create_velocity_visualizer()
        self.stepped_page_visualizer_container.addWidget(container)  # index 0 - velocity mode

        # tick mode page
        container = self._create_tick_visualizer()
        self.stepped_page_visualizer_container.addWidget(container)  # index 1 - stepped mode

        # encoder mode page
        container = self._create_encoder_visualizer()
        self.stepped_page_visualizer_container.addWidget(container)  # index 2 - encoder mode

        self._stepped_mode_map = {
            StepMode.Velocity: 0,
            StepMode.Stepped: 1,
            StepMode.Encoder: 2,
        }
        ...
        self._update_latched_vizualiser()  # select the proper page

    def _create_latched_selector(self):
        """creates the selector that allows the user to enable or disable latched input for the secondary action"""
        container, container_layout = gremlin.ui.ui_common.getVContainer()
        ...
        container_layout.addWidget(gremlin.ui.ui_common.QHorizontalLine())
        container_layout.addWidget(self.step_latched_enabled_widget)

        self.latch_configuration_container = QtWidgets.QWidget()
        sub_container_layout = QtWidgets.QVBoxLayout(self.latch_configuration_container)
        sub_container_layout.setContentsMargins(4, 0, 0, 0)
        container_layout.addWidget(self.latch_configuration_container)

        # latch mode selector
        latched_inputs = [
            ("Keyboard", InputType.KeyboardLatched, "Latched input is a keyboard sequence"),
            ("Joystick", InputType.JoystickButton, "Latched input is a joystick button"),
        ]
        widgets = ["Latched Input Type:"]
        for label, input_type, tooltip in latched_inputs:
            widget = gremlin.ui.ui_common.QDataRadioButton(
                label=label,
                data=input_type,
                value=self.action_data.latched_input_type == input_type,
                callbackEx=self._latched_input_type_changed,
                tooltip=tooltip,
            )
            widgets.append(widget)
        ...
        widgets.append("||")
        widget = gremlin.ui.ui_common.getHContainer(widgets, widget_only=True)
        sub_container_layout.addWidget(widget)

        # latch mode visualizers
        self.latched_visualizer_container = gremlin.ui.ui_common.QAutoResizeStackedWidget()
        sub_container_layout.addWidget(self.latched_visualizer_container)

        # latch mode selector (keyboard or joystick)
        listen_widget = gremlin.ui.ui_common.Buttons.getListenWidget(callback=self._stepped_listen)
        listen_keyboard_widget = gremlin.ui.ui_common.Buttons.getKeyboardWidget("Select Keys", callback=self._keyboard_dialog)

        self.latched_device_widget = QtWidgets.QWidget()
        self.latched_device_layout = QtWidgets.QVBoxLayout(self.latched_device_widget)

        # keyboard mode latch
        keyboard_container, keyboard_container_layout = gremlin.ui.ui_common.getVContainer()
        self.latched_visualizer_container.addWidget(keyboard_container)  # index 0

        # keyboard latch mode --------------------------
        keyboard_container_layout.addWidget(gremlin.ui.ui_common.getHContainer(["Latched Keys:", listen_keyboard_widget], widget_only=True))
        self.key_combination_widget, self.key_combination_layout = gremlin.ui.ui_common.getHContainer()
        keyboard_container_layout.addWidget(self.key_combination_widget)
        self._update_latched_keys()  # populate current latched keys
        ...
        for dev in devices:
            ...
            if button_list:
                ...
                device_items.append((dev.name, dev.device_id, f"Latched input device: {dev.name}"))

        dev = gremlin.joystick_handling.getDevice(self.action_data.stepped_device_id)
        if dev is None:
            # change to the first valid device
            dev = devices[0] if devices else None

        self.stepped_selector_device_widget = gremlin.ui.ui_common.QDataComboBox(
            source=device_items, value=self.action_data.stepped_device_id, callback=self._stepped_device_changed_cb
        )
        ...
        w1 = ["Reverse Device:", self.stepped_selector_device_widget, listen_widget]

        w2 = ["Type", self.stepped_selector_input_type_widget]

        w3 = ["Reverse Input:", self.stepped_selector_input_widget]

        w4 = ["Reverse Direction:", self.stepped_direction_widget]  # hat line

        device_selector_widget = QtWidgets.QWidget()
        device_selector_layout = QtWidgets.QVBoxLayout(device_selector_widget)

        widgets = [w1, w2, w3, w4]
        grids = []
        for w in widgets:
            grid = gremlin.ui.ui_common.getGridContainer(w, widget_only=True)
            device_selector_layout.addWidget(grid)
            grids.append(grid)
        gremlin.ui.ui_common.synchronize_grids(grids)

        self.latched_visualizer_container.addWidget(device_selector_widget)  # index 1
        self.stepped_hat_container = grids[-1]

        return container

    def _handle_direction_changed(self, direction):
        self.action_data.stepped_hat_direction = direction

    def _create_encoder_visualizer(self):
        container = QtWidgets.QWidget()
        container_layout = QtWidgets.QVBoxLayout(container)
        container_layout.addWidget(QtWidgets.QLabel("Encoder Mode Configuration:"))
        ...
        container_layout.addWidget(
            gremlin.ui.ui_common.getHContainer([self.encoder_direction_widget, self.encoder_mode_widget, self.encoder_spring_container], widget_only=True)
        )
        ...
        self.min_step_widget = gremlin.ui.ui_common.QFloatLineEdit(
            min_range=0,
            max_range=100,
            value=self.action_data.encoder_min_step,
        ...
        self.max_step_widget = gremlin.ui.ui_common.QFloatLineEdit(
            min_range=0,
            max_range=100,
            value=self.action_data.encoder_max_step,
    ...
    def _handle_full_frequency_changed(self, value):
        self.action_data.encoder_accel_full_hz = value

    def _handle_curve_power_changed(self, value):
        self.action_data.encoder_accel_curve = value

    def _handle_min_step_changed(self, value):
        self.action_data.encoder_min_step = value

    def _handle_max_step_changed(self, value):
        self.action_data.encoder_max_step = value

    def _handle_timeout_changed(self, value):
        self.action_data.encoder_frequency_timeout = value

    def _create_tick_visualizer(self):
        container = QtWidgets.QWidget()
        container_layout = QtWidgets.QVBoxLayout(container)
        container_layout.addWidget(QtWidgets.QLabel("Stepped (tick) Mode Configuration:"))
        ...
        self.normalize_widget = gremlin.ui.ui_common.QDataPushButton(
            "Normalize", callback=self._normalize_steps, tooltip="Normalizes steps to be evenly spaced"
        )

        self.low_progression_widget = gremlin.ui.ui_common.QDataPushButton(
            "Low", callback=self._low_progression_steps, tooltip="Steps follow a linear progression from the low range.  Most steps are in the lower half."
        )

        self.high_progression_widget = gremlin.ui.ui_common.QDataPushButton(
            "High", callback=self._high_progression_steps, tooltip="Steps follow a linear progression from the high range.  Most steps are in the higher half."
        )

        self.cubic_progression_low_widget = gremlin.ui.ui_common.QDataPushButton(
            "Geometric Low", callback=self._geometric_progression_steps_low, tooltip="Steps follow a geometric (log) progression."
        )

        self.cubic_progression_high_widget = gremlin.ui.ui_common.QDataPushButton(
            "Geometric High", callback=self._geometric_progression_steps_high, tooltip="Steps follow a geometric (log) progression."
        )

        self.cubic_progression_center_widget = gremlin.ui.ui_common.QDataPushButton(
            "Cubic Center", callback=self._cubic_steps_center, tooltip="Steps follow a cubic progression."
        )
        ...
        container_layout.addWidget(self.container_ticks_widget)
        container_layout.addStretch()

        return container

    def _create_velocity_visualizer(self):
        # velocity visualizer

        container = QtWidgets.QWidget()
        container_layout = QtWidgets.QVBoxLayout(container)
        container_layout.addWidget(gremlin.ui.ui_common.QHorizontalLine())
        container_layout.addWidget(QtWidgets.QLabel("Velocity Mode Configuration:"))
        ...
        container_layout.addWidget(self.container_linear_timings)

        container_layout.addStretch()

        return container

    def _latched_input_type_changed(self, widget, checked):
        if checked:
            self.action_data.latched_input_type = widget.data
            self._update_latched_vizualiser()

    def _update_latched_vizualiser(self):
        match self.action_data.latched_input_type:
            case InputType.KeyboardLatched:
                self.latched_visualizer_container.setCurrentIndex(0)
            case InputType.JoystickButton:
                self.latched_visualizer_container.setCurrentIndex(1)

    def _update_latched_keys(self):
        """displays latched keys for the latched input"""
    ...
    def _stepped_listen_ui(self):
        ...
        gremlin.shared_state.push_suspend_highlighting()
        button_press_dialog = gremlin.ui.ui_common.InputListenerWidget(return_kb_event=False)

        button_press_dialog.item_selected.connect(self._update_button)
        button_press_dialog.closed.connect(self._stepped_listen_closed_cb)
    ...
    def _stepped_listen_closed_cb(self, accepted: bool):
        """called when the button listen dialog is closed"""
        gremlin.shared_state.pop_suspend_highlighting()

    def _keyboard_dialog(self):
        """opens the keyboard dialog"""
        gremlin.shared_state.push_suspend_ui_keyinput()
        self._keyboard_dialog = InputKeyboardDialog(sequence=self.action_data.latched_keys, parent=self)
    ...
    def _keyboard_dialog_ok_cb(self):
        ...
        # grab the new data
        self.action_data.setKeys(gremlin.keyboard.sort_keys(self._keyboard_dialog.keys))
        self._update_latched_keys()
        gremlin.shared_state.pop_suspend_ui_keyinput()

    def _update_button(self, event: gremlin.event_handler.Event):
        gremlin.util.InvokeUiMethod(self._update_button_ui, event)

    def _update_button_ui(self, event_or_list: gremlin.event_handler.Event | list[gremlin.event_handler.Event]):
        """called when a button input is selected - runs on ui thread"""
        if isinstance(event_or_list, list):
            event = event_or_list[0]
        else:
            event = event_or_list
    ...
    @QtCore.Slot()
    def _stepped_device_changed_cb(self, device_id):
        """stepped device changed"""
        self.action_data.stepped_device_id = device_id

    def _update_stepped_selectors(self):
        device_id = self.action_data.stepped_device_id
        input_id = self.action_data.stepped_input_id

        dev = gremlin.joystick_handling.getDevice(device_id)
        input_types = []
        if dev.button_count:
            input_types.append(("Button", InputType.JoystickButton))
        if dev.hat_count:
            input_types.append(("Hat", InputType.JoystickHat))
        with QtCore.QSignalBlocker(self.stepped_selector_input_type_widget):
            self.stepped_selector_input_type_widget.clear()
            for name, input_type in input_types:
                self.stepped_selector_input_type_widget.addItem(name, input_type)
            index = self.stepped_selector_input_type_widget.findData(self.action_data.stepped_input_type)
            if index != -1:
                self.stepped_selector_input_type_widget.setCurrentIndex(index)
            else:
                # not found, reset the input type to the first
                self.action_data.stepped_input_type = input_types[0][1]
                self.stepped_selector_input_type_widget.setCurrentIndex(0)

        # populate hat or button ID list
        with QtCore.QSignalBlocker(self.stepped_selector_input_widget):
            match self.action_data.stepped_input_type:
                case InputType.JoystickButton:
                    self.stepped_selector_input_widget.setSource([(f"Button {i}", i) for i in range(1, dev.button_count + 1)])
                case InputType.JoystickHat:
                    self.stepped_selector_input_widget.setSource([(f"Hat {i}", i) for i in range(1, dev.hat_count + 1)])

            index = self.stepped_selector_input_widget.findData(input_id)
            if index != -1:
                self.stepped_selector_input_widget.setCurrentIndex(index)
            else:
                index = 0
                self.stepped_selector_input_widget.setCurrentIndex(index)
                self.action_data.stepped_input_id = self.stepped_selector_input_widget.itemData(index)
    ...
    def _stepped_input_type_changed_cb(self, input_type):
    ...
    def _handle_virtual_device_input_changed(self, index: int):
        ...
        with QtCore.QSignalBlocker(self.virtual_device_selector_widget):
            ...
            self.action_data.virtual_device = device
            self._update_device_input_list()
            self._update_hat_mapping()
            ...
            if connected:
                ...
                if device:
                    self.setWarning(f"Warning: Device VJOY [{device.virtual_id}] is not currently connected.")
                else:
        ...
        if self.virtual_output_selector_widget.count():
            ...
            with QtCore.QSignalBlocker(self.virtual_output_selector_widget):
                if (
                    ...
                    id = self.virtual_output_selector_widget.itemData(index)
    ...
    def _handle_tag_callback(self, action: VjoyRemap, extra_data: dict):  # noqa: F821
        if action.virtual_id == self.action_data.virtual_id:
            # same vjoy device
            input_type = (
    ...
    def notify_device_changed(self):
        ...
        event.vjoy_id = self.action_data.virtual_id
    ...
    def _update_device_input_list(self):
        ...
        with QtCore.QSignalBlocker(self.virtual_output_selector_widget):
            self.virtual_output_selector_widget.clear()
            ...
            if action_mode in (
                ...
                if action_mode in (
                    ...
                    count = device.button_count
                    for id in range(1, count + 1):
                        self.virtual_output_selector_widget.addItem(f"Button {id}", id)
    ...
    def _update_ui(self):
        ...
        match action:
            ...
            case VjoyAction.VJoySetAxisStepped:
                ...
                hat_visible = self.action_data.stepped_input_type == InputType.JoystickHat
                self.stepped_hat_container.setVisible(hat_visible)
        ...
        self.virtual_device_label_widget.setVisible(selector_visible)
        self.virtual_device_selector_widget.setVisible(selector_visible)
        self.virtual_output_selector_widget.setVisible(selector_visible)
        self.virtual_output_label_widget.setVisible(selector_visible)
        ...
        self.virtual_output_selector_widget.setVisible(input_selector_visible)
        self.virtual_output_label_widget.setVisible(input_selector_visible)
    ...
    def _handle_action_mode_changed(self, index: int):
        ...
        with QtCore.QSignalBlocker(self.cb_action_list):
            ...
            self.virtual_output_selector_widget.clear()  # ensure output is refreshed
            self._update_ui()
            self._update_device_input_list()
            self._update_merge_data()
            self._update_repeater()
    ...
    def _create_grid_widgets(self):
        ...
        if not self.action_data.virtual_device:
            # no device = no buttons
            return
        ...
        if self.button_grid_stack_widget.count() > 1:
            ...
            widget = self.button_grid_stack_widget.widget(1)
            widget.hide()
        ...
        for id in range(1, button_count + 1):
            ...
            marker = id in used_list
            used = id == self.action_data.input_id
            widget = gremlin.ui.ui_common.QUsedPushButton(
                str(id), id, used=used, marker=marker, callback=self._button_clicked, checkable=True, checked=self.action_data.input_id == id
            )
        ...
        self.button_grid_stack_widget.addWidget(grid_container_widget)
    ...
    def _populate_ui(self):
        ...
        if self.action_data.virtual_id not in [0, None]:
            vjoy_dev_id = self.action_data.virtual_id

        # Get the input type which can change depending on the container used
        input_type = self.action_data.input_type

        if self.action_data.container.tag == "hat_buttons":
        ...
        try:
            ...
            if self.action_data.grid_visible:
                self._populate_grid()
...
class VJoyRemapFunctor(gremlin.base_profile.AbstractFunctor):
    ...
    def __init__(self, action_data: VjoyRemap, parent=None):  # noqa: F821
        ...
        self.step_mode = action_data.step_mode
        ...
        self._pulse_active = False  # true if we're actively pulsing
        self._pulse_input_type = None
    ...
    def profile_start(self):
        ...
        if self.action_mode == VjoyAction.VJoySetAxisStepped:
            ...
            if self.action_data.encoder_mode == EncoderMode.Relative:
                ...
                self.encoder_target = 0.0  # center
    ...
    def profile_stop(self):
        ...
        if self._pulse_active:
            ...
            self.pulse_stop(self.vjoy_id, self._pulse_input_type, self.vjoy_input_id)
            self._pulse_input_type = None
    ...
    def _pulse_on(self, data):
        ...
        if self.action_data.action_mode == VjoyAction.VJoySetAxisStepped:
            ...
            if self.action_data._target_step_linear_mode:
                ...
                if self.verbose:
                    syslog.info(f"STEPPED AXIS: linear: direction: [{self.step_direction}] computednew value: [{value:0.3f}]")
            else:
    ...
    def pulse_start(
        ...
        if key in self.pulse_worker_map:
            ...
            worker = PulseWorker(duration, interval, self._pulse_on, self._pulse_off, iteration_callback=iteration_callback, data=args)
    ...
    def pulse_stop(
        ...
        key = (device_id, input_type, input_id, position)
        if key in self.pulse_worker_map:
            worker: PulseWorker = self.pulse_worker_map[key]
                    ...
                    if axis_mode == "absolute":
                        ...
                        if self.action_data.use_relative_value:
                            ...
                            if not self._relative_pulse_worker:
                                self._relative_pulse_worker = PulseWorker(
                                    0,
            ...
            match self.action_mode:
                ...
                case VjoyAction.VJoySetAxisStepped:
                    ...
                    if self.action_data._stepped_latched:
                        ...
                        match self.action_data.latched_input_type:
                            case InputType.JoystickButton:
                                # button or hat latching mode (joystick device)
                                if not fire_event:
                                    latched = (
                    ...
                    match self.action_data.step_mode:
                        case StepMode.Velocity:
                            ...
                            if latched or fire_event or self._pulse_active:
                                ...
                                if primary or latched:
                                    ...
                                    if trigger:
                                        ...
                                        if self.action_data._target_step_linear_mode:
                                            ...
                                            self._start_time = None
                                            self._start_value = self._get_axis(self.vjoy_id, self.vjoy_input_id)
                                            self._pulse_active = True
                                            self._pulse_input_type = input_type
                                            if verbose:
                                                syslog.info(
                                            ...
                                            if verbose:
                                                syslog.info("\tPulse iterated")
                            else:
                                # release velocity mode - stop pulsing
                                if self._pulse_active:
                                    # stop the updates
                                    if verbose:
...
class MergeData:
    ...
    def compute(self, value: float):
        ...
        assert isinstance(value, float), "invalid value: should be a float"
...
class VjoyRemap(gremlin.input_item.AbstractAction):
...
"""
    ...
    def __init__(self, container, extra_data: dict = None):
        ...
        self.step_mode = StepMode.Stepped  # default stepping mode when action mode is set to VjoyAction.VJoySetAxisStepped

        # encoder configuration
        self.encoder_mode = EncoderMode.Relative  # default encoder mode
        self.encoder_tick = 0.01  # default encoder tick value
        self.encoder_direction = 1  # default encoder direction (clockwise)
        self.encoder_accel_start_hz = 2.0  # default encoder acceleration start frequency (pulses per second)
        ...
        self._latched_keys = []  # secondary input latched keys
        self.latched_input_type = InputType.KeyboardLatched
        ...
        self.stepped_hat_direction: tuple = (0, 0)  # hat position of the down step action to latch when mapping to a hat input
    ...
    def latched_keys(self):
        if self.latched_input_type == InputType.KeyboardLatched:
            return self._latched_keys
        # no latched keys in button mode
        return []
    ...
    def addKey(self, key):
        if key not in self._latched_keys:
            self._latched_keys.append(key)

    def removeKey(self, key):
        if key in self._latched_keys:
            self._latched_keys.remove(key)
    ...
    def refresh_virtual_inputs(self):
        """updates virtual input devices device map"""
        self.virtual_device_map = gremlin.joystick_handling.virtual_device_map()
        if self.virtual_device is None:
            # pick the first one if it exists
            for device_type in self.virtual_device_map:
                if self.virtual_device_map[device_type]:
                    for device in self.virtual_device_map[device_type].values():
    ...
    def get_merged_axis_value(self, event):
        """computes the merged axis value for the current configuration"""
        ...
        if self.action_mode == VjoyAction.VJoyMergeAxis:
            ...
            # this handles axis merging and applies any curves and axis inversion
            filtered_value = self.get_filtered_axis_value(value, curves=curves)
            if filtered_value is None:
                filtered_value = value

            ranged_value = self.get_ranged_axis_value(filtered_value)

            if ranged_value is None:
                ranged_value = filtered_value

            action_value = gremlin.actions.Value(value=ranged_value, raw=event.raw_value, is_pressed=event.is_pressed)
            merged_value = action_value.current
            for data in self._merge_data:
                merged_value = data.compute(merged_value)
            return merged_value
        return event.value
    ...
    def reverse(self, value):
        self.usage_data.set_inverted(self.virtual_device_guid, self.vjoy_axis_id, value)
        self._reverse = value
    ...
    def _parse_xml(self, node, data=None, extra_data=None):
        ...
        try:
            device_type = safe_read(node, "device-type", str, "").casefold()
            if not device_type:
            ...
            if virtual_id != -1:
                device_map = gremlin.joystick_handling.virtual_device_map()
                if device_type in device_map:
                    if virtual_id in device_map[device_type]:
            ...
            self.step_mode = StepMode.Stepped  # default
            if "step-mode" in node.attrib:
                value = node.attrib["step-mode"]
                step_mode = StepMode.from_string(value)
                if step_mode:
                    self.step_mode = step_mode

            if "encoder-mode" in node.attrib:
                value = node.attrib["encoder-mode"]
                self.encoder_mode = EncoderMode.from_string(value)
            else:
                self.encoder_mode = EncoderMode.Relative  # default
            if "encoder-tick" in node.attrib:
                self.encoder_tick = safe_read(node, "encoder-tick", float, 0.01)
            if "encoder-direction" in node.attrib:
                self.encoder_direction = safe_read(node, "encoder-direction", int, 1)
            if "encoder-accel-start-hz" in node.attrib:
                self.encoder_accel_start_hz = safe_read(node, "encoder-accel-start-hz", float, 2.0)
            if "encoder-accel-full-hz" in node.attrib:
                self.encoder_accel_full_hz = safe_read(node, "encoder-accel-full-hz", float, 30.0)
            if "encoder-accel-curve" in node.attrib:
                self.encoder_accel_curve = safe_read(node, "encoder-accel-curve", float, 2.0)
            if "encoder-frequency-filter" in node.attrib:
                self.encoder_frequency_filter = safe_read(node, "encoder-frequency-filter", float, 0.0)
            if "encoder-direction-change-speed-retention" in node.attrib:
            ...
            if "encoder-max-step" in node.attrib:
                self.encoder_max_step = safe_read(node, "encoder-max-step", float, 0.1)

            # latching
            keys = []
            key_nodes = node.xpath(".//latched-key")
            ...
            if "latched-input-type" in node.attrib:
                value = node.attrib["latched-input-type"]
                self.latched_input_type = InputType.from_string(value)
            else:
                # default latch mode
            ...
            match self.latched_input_type:
                case InputType.Keyboard:
                    ...
                    self.stepped_input_type = InputType.Keyboard
                case InputType.JoystickButton:
                    if "stepped-device-id" in node.attrib:
                        self.stepped_device_id = safe_read(node, "stepped-device-id", str, "")
            ...
            if "stepped-hat-position" in node.attrib:
                self.stepped_hat_direction = vjoy.vjoy.Hat.getDirection(node.get("stepped-hat-position"))
    ...
    def _generate_xml(self):
        ...
        if self.action_mode in (
            ...
            node.set(InputType.to_string(self.input_type), safe_format(output_id, int))
            ...
            case VjoyAction.VJoySetAxisStepped:
                ...
                match self.step_mode:
                    ...
                    case StepMode.Velocity:
                        ...
                        node.set("pulse_delay", safe_format(self.pulse_delay, int))
                    case StepMode.Encoder:
                        node.set("encoder-mode", safe_format(EncoderMode.to_string(self.encoder_mode), str))
                        node.set("encoder-tick", safe_format(self.encoder_tick, float))
                        node.set("encoder-direction", safe_format(self.encoder_direction, int))
                        node.set("encoder-accel-start-hz", safe_format(self.encoder_accel_start_hz, float))
                        node.set("encoder-accel-full-hz", safe_format(self.encoder_accel_full_hz, float))
                        node.set("encoder-accel-curve", safe_format(self.encoder_accel_curve, float))
                        node.set("encoder-frequency-filter", safe_format(self.encoder_frequency_filter, float))
                        node.set("encoder-direction-change-speed-retention", safe_format(self.encoder_direction_change_speed_retention, float))
        ...
        if self.latched_input_type in (InputType.Keyboard, InputType.KeyboardLatched):
            if self._latched_keys:
                for key in self._latched_keys:
                    ...
                    key_node = ElementTree.SubElement(node, "latched-key")
                    key_node.set("virtual-code", str(key.virtual_code))
                    key_node.set("scan-code", str(key.scan_code))
                    key_node.set("extended", str(key.is_extended))
        ...
        if VjoyAction.is_button_action(self.action_mode):
            node.set("button", safe_format(self.vjoy_button_id, int))
    ...
    def to_html(self) -> str:
        ...
        table.addField("Vjoy Device", self.virtual_id)
        ...
        match self.action_mode:
            ...
            case VjoyAction.VJoyHatToButton:
                for position in self.hat_map:
                    input_id = self.hat_map[position]
    ...
    def report_record(self) -> tuple:
        ...
        label += f"| VJoy ID | {self.virtual_id}"
    ...
    def __str__(self):
        ...
        return f"VjoyRemap: VJOY device: {self.virtual_id} {input_string}: {self.vjoy_input_id}"
