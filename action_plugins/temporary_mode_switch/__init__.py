...
class TemporaryModeSwitchFunctor(gremlin.base_profile.AbstractFunctor):
    ...
    def process_event(self, event, value, extra_data=None):
        ...
        if trigger:
            ...
            if verbose:
                syslog.info(
                    f"Temporary mode change: [{current_mode}] - next mode: [{next_mode}] - return mode: [{return_mode}] - specific mode to return: [{'enabled' if self.action_data.enable_return_mode else 'disabled'}]"
                )
            ...
            if verbose:
                ...
                syslog.info(
                    f"TEMPORARY MODE CHANGE ACTION: [{self.id}] requesting mode switch from [{current_mode}] to [{new_mode}] and back to [{return_mode}]"
                )
    ...
    def _restore_callback(self, args):
        """callback on button release"""

        next_mode, return_mode = args
        verbose = gremlin.config.Configuration().verbose_mode_mode
        ...
        if next_mode != current_mode:
            ...
            if verbose:
                ...
                syslog.info(
                    f"TEMPORARY MODE CHANGE CALLBACK: [{self.id}] current mode [{current_mode}] is incorrect - callback only works on mode [{next_mode}] - FAIL"
                )
                syslog.info(f"\tAttached device: [{device_name}] input type: [{InputType.to_display_name(input_type)}] input: [{input_id}] mode: [{mode}]")
            return
        ...
        if verbose:
            ...
            syslog.info(f"TEMPORARY MODE CHANGE CALLBACK: [{self.id}] requesting mode switch from [{current_mode}] to [{return_mode}] - SUCCESS")
            syslog.info(f"\tAttached device: [{device_name}] input type: [{InputType.to_display_name(input_type)}] input: [{input_id}] mode: [{mode}]")
            syslog.info(f"\tcurrent profile mode: {gremlin.shared_state.runtime_mode} mode to set: {self.action_data.mode}")

        gremlin.event_handler.EventHandler().change_mode(return_mode)


class TemporaryModeSwitch(gremlin.input_item.AbstractAction):
