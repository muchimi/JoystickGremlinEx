...
import html
from gremlin.types import KeyboardOutputMode, SyncMode, SendType
...
class MapToKeyboardExWidget(gremlin.input_item.AbstractActionWidget):
    ...
    def _create_ui(self):
        ...
        self.listen_multi_widget.setToolTip(
            "Listen for multiple inputs.  Click on ok when done.  Clicks on the buttons will not be included in the recorded list."
        )
        ...
        self.show_keyboard_widget = gremlin.ui.ui_common.QIconPushButton("Select...")
        ...
        self.show_keyboard_widget.setFixedHeight(24)  # match the other buttons
        ...
        self.container_options_widget = gremlin.ui.ui_common.getGridContainer(
            gremlin.ui.ui_common.getGridContainer(widgets, widget_only=True), "Output Mode:", widget_only=True, left_margin=12
        )
        ...
        widgets = [self.clear_widget, self.listen_widget, self.listen_multi_widget, self.show_keyboard_widget]
        ...
        self._sync_widget = gremlin.ui.ui_common.QSyncModeWidget(
            mode=self.action_data.sync_mode, label="State on profile start:", sync_modes=[SyncMode.Ignore, SyncMode.Input], callback=self._sync_changed
        )
        ...
        widget = gremlin.ui.ui_common.QIntLineEdit(value=self.action_data.wheel_factor, min_range=1, max_range=100, callback=self._handle_wheel_factor_changed)
    ...
    def _update_ui(self):
        ...
        match mode:
            case KeyboardOutputMode.Press:
                description = (
                    "<b>Press</b> mode will press keys() - the key(s) is not released and should be paired with a release at some point (keyboard 'make')."
                )
    ...
    def _record_keys_ui(self, multi_keys):
        ...
        button_press_dialog.setGeometry(int(geom.x() + geom.width() / 2 - 150), int(geom.y() + geom.height() / 2 - 75), 300, 150)
...
class MapToKeyboardExFunctor(gremlin.base_profile.AbstractFunctor):
    def __init__(self, action, parent=None):
        ...
        if verbose:
            ...
            if self.use_macros:
                if verbose:
                    syslog.info(f"{logtabs}KEY ACTION: DelayPressMacro:")
                for key in action.keys:
                    if verbose:
                        syslog.info(f"{logtabs}\tPress: {key}")
                    self.delay_press_release.press(key)
                if self.delay > 0:
                    if verbose:
                        syslog.info(f"{logtabs}\tPause: {self.delay}")
                    self.delay_press_release.pause(self.delay)
                for key in reversed(action.keys):
                    if verbose:
                        syslog.info(f"{logtabs}\tRelease: {key}")
    ...
    def profile_start(self):
        ...
        if remote_config.isProcess:
            ...
            self.extra_data = {"process_name": remote_config.process, "partial_match": remote_config.partialMatch}
        ...
        match self.action_data.sync_mode:
            ...
            case SyncMode.Input:
                if self.verbose:
                    syslog.info(f"\t sync to input : {is_pressed}")
            ...
            if self._hold_keys:
                ...
                if self.use_macros:
                    ...
                    for key in self._hold_keys:
                        ...
                        gremlin.keyboard.send_key_up(
                            key, is_local, is_remote, client_list=self.client_list, target_hwnd=self.target_hwnd, extra_data=self.extra_data
                        )
    ...
    def _manual_release(self):
        ...
        for key in self._release_keys:
            if key.is_mouse:
                ...
                gremlin.keyboard.send_key_up(key, is_local, is_remote, client_list=self.client_list, target_hwnd=self.target_hwnd, extra_data=self.extra_data)
    ...
    def _pulse_on(self, data):
        ...
        for key in press_keys:
            if self.verbose:
                syslog.info(f"Pulse ON [{key.debug_name}]")
            gremlin.keyboard.send_key_down(
                key, is_local, is_remote, client_list=self.client_list, target_hwnd=self.target_hwnd, extra_data=self.extra_data
            )  # handles local and remote and special mouse keys
    ...
    def _pulse_off(self, data):
        ...
        for key in release_keys:
            if self.verbose:
                syslog.info(f"Pulse OFF [{key.debug_name}]")
            gremlin.keyboard.send_key_up(
                key, is_local, is_remote, client_list=self.client_list, target_hwnd=self.target_hwnd, extra_data=self.extra_data
            )  # handles local and remote and special mouse keys
    ...
    def pulse_start(self, data: tuple, duration: float, interval: float):
        """pulse setup"""
        if self.verbose:
            syslog.info(f"Pulse START keyboard [{self.id}] duration: {duration:0.3f} interval: {interval:0.3f}")
        key = self.id
    ...
    def pulse_stop(self):
        ...
        if self.verbose:
            syslog.info(f"Pulse STOP keyboard [{self.id}]")
        key = self.id
        if key in self.pulse_worker_map:
            ...
            match mode:
                case KeyboardOutputMode.Release:
                    ...
                    if not self.has_keys:
                        ...
                        if self.use_macros:
                            ...
                            for key in self._release_keys:
                                if verbose:
                                    syslog.info(f"KEYBOARD: press: send key release: {key}")
                                if key.is_mouse:
                                    gremlin.macro._send_mouse_button(
                                        key.mouse_button, False, is_local, is_remote, wheel_factor=self.action_data.wheel_factor, client_list=self.client_list
                                    )
                                else:
                                    gremlin.keyboard.send_key_up(
                                        key, is_local, is_remote, client_list=self.client_list, target_hwnd=self.target_hwnd, extra_data=self.extra_data
                    ...
                    if self.has_keys:
                        ...
                        if self.use_macros:
                            ...
                            for key in self._press_keys:
                                if verbose:
                                    syslog.info(f"KEYBOARD: press: send key press: {key}")
                                if key.is_mouse:
                                    gremlin.macro._send_mouse_button(
                                        key.mouse_button, True, is_local, is_remote, wheel_factor=self.action_data.wheel_factor, client_list=self.client_list
                                    )
                                else:
                                    gremlin.keyboard.send_key_down(
                                        key, is_local, is_remote, client_list=self.client_list, target_hwnd=self.target_hwnd, extra_data=self.extra_data
                    ...
                    if self.has_keys:
                        ...
                        if is_pressed and not auto_release:
                            ...
                            if self.use_macros:
                                ...
                                for key in self._press_keys:
                                    if verbose:
                                        syslog.info(f"KEYBOARD: hold: send key press: {key}")
                                    if key.is_mouse:
                                        gremlin.macro._send_mouse_button(
                                            key.mouse_button,
                                            True,
                                            is_local,
                                            is_remote,
                                            wheel_factor=self.action_data.wheel_factor,
                        ...
                        if is_pressed and auto_release:
                            if self.use_macros:
                                ...
                                def callback():
                                    return gremlin.macro.MacroManager().queue_macro(self.release, is_local, is_remote, client_list=self.client_list)
                                ...
                                for key in self._release_keys:
                                    if verbose:
                                        syslog.info(f"send key press (autorelease): {key}")
                                    if key.is_mouse:
                                        gremlin.macro._send_mouse_button(
                                            key.mouse_button,
                                            True,
                ...
                case KeyboardOutputMode.Pulse:
                    ...
                    if self.has_keys:
                        if is_pressed:
                            if verbose:
                                syslog.info("MapToKeyboardEx: pulse")
                ...
                case KeyboardOutputMode.Toggle:
                    if self.has_keys:
                        if is_pressed:
                            for key in self._press_keys:
                                if verbose:
                                    syslog.info("MapToKeyboardEx: toggle")
                                el = gremlin.event_handler.EventListener()
            ...
            if self.has_keys:
                match mode:
                    case KeyboardOutputMode.Hold:
                        if self.use_macros:
                            if verbose:
                                syslog.info("HOLD: send key releases (macro mode)")
                            gremlin.macro.MacroManager().queue_macro(self.release)
                        else:
                            # not using macros
                            for key in self._release_keys:
                                if verbose:
                                    syslog.info(f"HOLD: send key release: {key}")
                                gremlin.keyboard.send_key_up(
                                    key, is_local, is_remote, client_list=self.client_list, target_hwnd=self.target_hwnd, extra_data=self.extra_data
                                )
...
Can also send mouse buttons, mouse wheel events."""
    ...
    @keys.setter
    def keys(self, value):
        raise AttributeError("Cannot set keys directly.")
    ...
    def _parse_xml(self, node, data=None, extra_data=None):
        ...
        if not keys:
            ...
            syslog.warning(
                f"MapToKeyboardEx: device: [{input_item.device_name if input_item else 'n/a'}] input: [{input_item.display_name if input_item else 'n/a'}] No keys found in XML.  This action will not do anything.: source line: [{node.sourceline}]"
            )
            pass

        # sort the keys for display purposes
        self._keys = gremlin.keyboard.sort_keys(keys)
        ...
        self.fireIconChanged()  # notify the UI that the icon may have changed
