...
class RemapWidget(gremlin.input_item.AbstractActionWidget):
    ...
    def _create_ui(self):
        ...
        input_types = {
            ...
            InputType.OpenSoundControl: [InputType.JoystickAxis, InputType.JoystickButton],
        ...
        self.vjoy_selector = ui_common.VJoySelector(
            ...
            self.action_data.get_settings().getVjoyAsInputList(),
        )
        ...
        warning_widget = gremlin.ui.ui_common.QIconLabel(
            ...
            text="Legacy mapper - consider using <i>VJoy Remap</i> for additional functionality",
            use_wrap=False,
        )
    ...
    def _notify_button_changed(self):
        if self.action_data.input_type == InputType.JoystickButton:
            ...
            device_guid = gremlin.joystick_handling.getVjoyDeviceGuid(vjoy_id)
...
Use Vjoy Remap instead."""
    ...
    def input_type(self, value: InputType):
        ...
        if self._input_type != value:
            if self._input_type == InputType.JoystickButton:
                el = gremlin.event_handler.EventListener()
                el.vjoy_button_usage.emit(device_guid, self._vjoy_input_id, False, self.id)

            self._input_type = value

            if self._input_type == InputType.JoystickButton:
                el = gremlin.event_handler.EventListener()
                el.vjoy_button_usage.emit(device_guid, self._vjoy_input_id, True, self.id)
    ...
    def _parse_xml(self, node, data=None, extra_data=None):
        ...
        try:
            ...
            if "axis" in node.attrib:
                ...
                raise gremlin.error.GremlinError(f"Invalid remap type provided: {node.attrib}")
    ...
    def _generate_xml(self):
        ...
        if self.input_type == InputType.Keyboard:
            node.set(InputType.to_string(InputType.JoystickButton), str(self.vjoy_input_id))
        else:
