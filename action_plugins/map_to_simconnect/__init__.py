...
class SimconnectAicraftDefinition:
    ...
    def __init__(
        ...
        if self.entry_type == SimconnectAicraftDefinition.EntryType.Scan:
            assert (
                community_path and aircraft_path
...
class SimconnectMonitor:
    ...
    def _profile_start(self):
        ...
        if enabled:
            ...
            eh.registerModeValidator(
                self._mode_change_validator
            )  # filter mode change requests and discard them if needed to avoid interrupting Simconnect activities
...
class SimconnectOptionsUi(gremlin.ui.ui_common.QRememberDialog):
    ...
    def __init__(self, simconnect: SimConnect, parent=None):
        ...
        self.scan_aircraft_widget.setToolTip(
            "Scan MSFS aicraft folders for aircraft names"
        )
        ...
        self.refresh_aircraft_widget.setToolTip(
        ...
        self.manual_container_map_layout = QtWidgets.QVBoxLayout(
            self.manual_container_map_widget
        )
        ...
        self.manual_scroll_area.setHorizontalScrollBarPolicy(
    ...
    def _update_scanned_list(self):
        ...
        for item in self._data[start_index:end_index]:
            ...
            # manufacturer
            manufacturer_widget = gremlin.ui.ui_common.QDataLineEdit(
                data=(item, selected_widget)
            )
            manufacturer_widget.setReadOnly(True)
            manufacturer_widget.setText(
                item.icao_manufacturer if item.icao_manufacturer else "n/a"
            )
            manufacturer_widget.installEventFilter(self)

            # model
            model_widget = gremlin.ui.ui_common.QDataLineEdit(
                data=(item, selected_widget)
            )
            ...
            # type
            type_widget = gremlin.ui.ui_common.QDataLineEdit(
                data=(item, selected_widget)
            )
            ...
            if event.is_axis and output_mode in (
                ...
                if process_input:
                    ...
                    if command_type == SimConnectCommandType.LVar:
                        ...
                        if verbose:
                            syslog.info(
                                f"SIMCONNECT: send lvar (axis): {command} input: {action_value.current:0.3f} scaled: {normalized:0.3f} curved: {curved:0.3f} min: {self.action_data.output_min_range:0.3f} max: {self.action_data.output_max_range:0.3f} -> scaled: {output_value:0.3f}"
                            )
                        ...
                        if verbose:
                            syslog.info(
                                f"SIMCONNECT: send simvar (axis): {block.command} input: {action_value.current:0.3f} scaled: {normalized:0.3f} curved: {curved:0.3f} min: {self.action_data.output_min_range:0.3f} max: {self.action_data.output_max_range:0.3f} -> scaled: {output_value:0.3f}"
                            )
