...
class Request(object):
    ...
    def _ensure_def(self):
        ...
        data_type = SIMCONNECT_DATATYPE.SIMCONNECT_DATATYPE_FLOAT64  # noqa: F405
        if ":index" in str(self.definitions[0][0]):
            self.lastIndex = b":index"
            return False
        ...
        if s_rtype.casefold() == "string":
            rtype = None
            data_type = SIMCONNECT_DATATYPE.SIMCONNECT_DATATYPE_STRINGV  # noqa: F405
        ...
        if self.is_client_data:
            ...
            err = self._sm._dll.AddToClientDataDefinition(
                ...
                4096,  # buffer size
                0,  # epsilon
                SIMCONNECT_UNUSED,  # datum  # noqa: F405
            )
            ...
            err = self._sm._dll.AddToDataDefinition(
                ...
                data_type,
                0,
                SIMCONNECT_UNUSED,  # noqa: F405
            )
        if self._sm.IsHR(err, 0):
            self.defined = True
            temp = DWORD(0)  # noqa: F405
            self._sm._dll.GetLastSentPacketID(self._sm._hSimConnect, temp)
            self.LastID = temp.value
            if verbose:
                syslog.info(f"SIMCONNECT: created request defintion OK: {command}  definition ID: {self.DATA_DEFINITION_ID.value}")
            return True
        else:
...
class SimConnect:
    ...
    def __init__(self, handler: SimConnectEventHandler, auto_connect=True):
        ...
        self._hSimConnect = HANDLE()  # noqa: F405
        self._running = False
        self._ok = False
    ...
    def handle_id_event(self, event: SIMCONNECT_RECV_EVENT):
        ...
        if uEventID == self._dll.EventID.EVENT_SIM_START.value:
            ...
            # aircraft request
            if self.verbose:
                syslog.info("SIMCONNECT: event: AIRCRAFT REQUEST")
            pass
    ...
    def handle_simobject_event(self, ObjData):
        ...
        if dwRequestID in self.Requests:
            ...
            if "string" in rtype.lower():
                ...
                _request.buffer = cast(ObjData.dwData, POINTER(c_double * len(_request.definitions))).contents[0]
    ...
    def simconnect_dispatch_proc(self, pData, cbData, pContext):
        ...
        if dwID == SIMCONNECT_RECV_ID.SIMCONNECT_RECV_ID_EVENT:
            ...
            _dwRequestID = pObjData.dwRequestID
    ...
    def add_to_notification_group(self, group, event, maskable: bool = False):
        if self._dll is not None:
            self._dll.AddClientEventToNotificationGroup(self._hSimConnect, group, event, maskable)
    ...
    def clear(self, request):
        ...
        if self._dll is not None and request and request.DATA_DEFINITION_ID:
            self._dll.ClearClientDataDefinition(self._hSimConnect, request.DATA_DEFINITION_ID.value)
    ...
    def add_waypoints(self, _waypointlist):
        ...
        if self.DEFINITION_WAYPOINT is None:
            ...
            _err = self._dll.AddToDataDefinition(
                self._hSimConnect,
        ...
        dataarray = (ctypes.c_double * len(pyarr))(*pyarr)
        _pObjData = cast(dataarray, c_void_p)
        _sx = int(sizeof(ctypes.c_double) * (len(pyarr) / len(_waypointlist)))
        return
    ...
    def get_paused(self):
        if self._dll is None:
            return False
        _hr = self._dll.RequestSystemState(self._hSimConnect, self._dll.EventID.EVENT_SIM_PAUSED, b"Sim")
    ...
    def createSimulatedObject(self, name, lat, lon, rqst, hdg=0, gnd=1, alt=0, pitch=0, bank=0, speed=0):
        ...
        self._dll.AICreateSimulatedObject(self._hSimConnect, name.encode(), simInitPos, rqst.value)
