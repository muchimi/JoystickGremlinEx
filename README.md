...
### (m77T51)
- Change: API Voice: (experimental) - additional UI elements (disabled in production)
- Fix: OBS Overlay: (experimental) - enhancements, thread safety and various fixes (courtesy Lolo350)
- Fix: Streamdeck Device: (experimental) - polish + paste/splitter stability.
...
- New: API Voice: (experimental) API level support for voice input/speech commands.  This module while not yet in production in T50 as UI features are currently disabled in this build while the internal testing continues and includes missing features, the API includes local services for speech recognition. This will follow in the coming days with a voice input device to use natural speech as an input device.  Voice commands will trigger mappings as any other input device would in GEX.
- Fix: Streamdeck Device: added compatibility for T50 (courtesy Lolo350).
...
- Fix: PlaySound Action: persist suppress duplication options and timings and update UI visuals.
...
- Fix: TTS API: incorrect volume could generate audio with zero volume (causing no sound output).
...
- New: Map to VJoy action: cubic distribution for stepped mode
- Change: Execution engine: reworked multi-key keyboard triggers, and is greatly simplified from the prior API.
...
- Fix: API: parent mode lookup for input matching when using nested modes - PRR by RazOrLegend (thank you!)
- Fix: UI: tray icon updated when profile is active
...
- Fix: LOG: verbose mode for axis lookup on compound input types unable to get virtual axis #.
- Fix: LOG: attach new verbosity elements to verbosity modes in m77.
- Fix: Chain Container: invalid type referenced due to incomplete automatic name refactor of condition objects.
- Fix: 1:1 mapping: vjoy remap action set to vjoy 1 regardless of selected vjoy device.
