...
- New: API Voice: (experimental) API level support for voice input/speech commands.  This module while not yet in production in T50, API includes local services for speech recognition. This will follow in the coming days with a voice input device to use natural speech as an input device.  Voice commands will trigger mappings as any other input device would in GEX.
